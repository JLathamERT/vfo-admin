...
| **Deposit refund issued** — the deposit was refunded via Stripe after the admin picked **Refund** on the "Tax Plan Green/Red Light" step (inside Tax 1 - Diagnostic; `task_order=6` since the 2026-08-07 reorder, previously last); the confirmation email drafted to the client carries the admin-typed reason. | FYI | Assigned PF | Admin Send Refund button — instant |
