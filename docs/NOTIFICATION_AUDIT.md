...
| **Deposit refund issued** — the deposit was refunded via Stripe after the admin picked **Refund** on the "Tax Plan Green/Red Light" step (last step of Tax 1 - Diagnostic); the confirmation email drafted to the client carries the admin-typed reason. | FYI | Assigned PF | Admin Send Refund button — instant |
