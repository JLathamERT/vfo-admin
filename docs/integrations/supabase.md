...
| `accountant-onboarding-agreements` | public | `actions/accountant/decision.ts` (Undecided branch) | Holds the static **Accountant Onboarding Agreement** PDF (`Accountant_Implementation_Agreement.pdf` — currently empty). Same fetch-and-attach pattern as advisor; same graceful fallback when PDF is missing. Created 2026-05-28. |
