...
| [`src/main.jsx`](../../src/main.jsx) | `Sentry.init({ dsn, environment: 'production' })` at startup, before the app renders |
...
`Sentry.init` passes **only** the DSN + `environment`. There is intentionally:
...
So Sentry receives error events only (the global handler plus the explicit `captureException` from the error boundary). It does not see normal user activity, network payloads, or session data.
