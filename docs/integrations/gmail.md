...
- Builds via `draftGmail` / `getGmailAccessToken` (same `utils/gmail-draft.ts` helpers as other handlers); placeholders include `[MEMBER_NAME]`, `[MEMBER_NUMBER]`, `[MEMBER_EMAIL]`, `[AGREEMENT_NAME]`, `[SELECTED_PLANS]`, `[PAYMENT_PLAN]`, `[INVOICE_NUMBER]`, `[TRANSITION_DATE]`, `[CLOSER]`, `[SETTER]`, `[INTRODUCED_BY]`, `[MEMBER_INTRODUCTION]`, and conditional `[COMPANY_BULLET]` / `[WEBSITE_BULLET]` (rendered only when the optional Company/Website fields were filled in the New Model Sale modal).
