...
| **Initial retainer** | On a revised-process tax plan above **$30,000**, the flat **$15,000** first of the two retainer payments. Stored as `client_tax_plans.initial_retainer_amount`; NULL on 2-payment and legacy plans. See [flows/tax-fee-process.md](flows/tax-fee-process.md). (2026-08-25) |
| **Final retainer** | The second retainer payment on such a plan — `retainer_amount − 15000`, i.e. the remainder of the 50% retainer. Stored as `client_tax_plans.final_retainer_amount`. Collected **off-session** when the client clicks the green Client decision 1 button, and its settlement is what releases the retainer revenue share. **`isThreePaymentPlan` is keyed on THIS column, not on `initial_retainer_amount`** (gotcha **#440**). See [flows/tax-fee-process.md](flows/tax-fee-process.md). (2026-08-25) |
...
| **Conversion (tax fee)** | A Tax 4 amendment to **≤ $30,000** that turns a 3-payment plan back into a 2-payment one by nulling `final_retainer_amount` while KEEPING `initial_retainer_amount` as the reversibility marker. Reversible above $30,000 until Client decision 1 is sent. See [flows/tax-fee-process.md](flows/tax-fee-process.md). (2026-08-25) |
