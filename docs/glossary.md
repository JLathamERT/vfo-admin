...
| **Team Member Responsible** | The Stage 1 step on Advisor + Accountant Onboarding that records which internal team member owns the onboarding. | A dropdown of five sales-team names (`SALES_TEAM_NAMES` in the frontend / `TEAM_MEMBER_NAMES` in `constants/onboarding-team.ts`). Required before the admin can pick a Decision (the Yes/Undecided/No buttons stay disabled until it is set). The chosen name routes that onboarding's notifications — 96h stalls, the "ready to create" alert, and the clicked-Yes alert — to that person's notification bell instead of the shared `admin` bell, via `teamMemberRecipient()` (name → `@elitert.com` login email). Stored on `{advisor,accountant}_onboarding.onboarding_team_member` + `onboarding_team_member_at`. |
