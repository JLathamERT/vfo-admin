...
| **Tax Planner** | An admin-managed person-type (table `tax_planners`) attached to a tax engagement — the "Advanced Tax Planner" allocated to a `client_tax_plans` row who earns the **Tax Planner Share** leg of the 3-way tax split. Mirrors Specialists (`experts`): admin CRUD + document vault + certifications, but **no portal login** (every surface is admin-only). Lives in the NEW "Tax Planners" 5th key admin section (2026-07-21). A planner's `member_type` names their **Tax Planning Group**, which is where their share is actually paid. |
