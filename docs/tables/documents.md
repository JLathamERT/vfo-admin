...
| `to_list` | jsonb | nullable. **Added 2026-06-15.** The "Team (To)" recipient list — an array of email addresses. Used ONLY by the `TEAM`/`new_model_sale` row; `utils/new-model-sale-email.ts` reads it as the draft's `To` (no Cc/Bcc). Editable via the Team Notification section of the Email Templates tab; `automation_save_email_template` sanitizes/dedupes on write. |
| `active` | boolean | default `true` |
| `created_at` | timestamptz | default `now()` |
