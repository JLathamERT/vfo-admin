...
| `to_list` | jsonb | nullable. **Added 2026-06-15; generalized 2026-07-03.** The email's To config — an array mixing raw emails and UPPERCASE role tokens (`CLIENT`, `MEMBER`, `ASSIGNED_PF`, `SPECIALIST`, `ADVISOR`, `ACCOUNTANT`, `TEAM_MEMBER`, `TEAM`, `RECIPIENT`), resolved per-send by `utils/email-recipients.ts resolveTemplateRecipients()`. Same token support applies to `cc_list`/`bcc_list`. Seeded for all rows by migration `email_templates_recipient_seeds`; edited as chips in the Email Templates tab; `automation_save_email_template` sanitizes (emails validated, known tokens kept). Empty/unresolvable To falls back to the handler's built-in target. **Blanket internal Cc (2026-07-31, DML only):** every row in pipelines `MAP 1` (34), `TAX` (58 — both tax programs), `PIP` (5) and `PARTNERSHIP_FAST_TRACK` (10) — **107 rows** — carries `tnmiller@elitert.com` + `tvaldes@elitert.com` in `cc_list`. **A NEW template in those pipelines does not inherit it** — the Cc lives in the row, so seed it explicitly. The typo address `tracy@vfo-services.com` (never existed; Tracy is `tnmiller@elitert.com`) was removed from ids 10/115/25/137 in the same pass and now appears nowhere. See [integrations/gmail.md](../integrations/gmail.md). |
| `active` | boolean | default `true` |
