...
One row per notification (and per sweep reminder-email tier), edited in **Admin Portal → Automation → Notification Editor** (`NotificationEditorPanel.jsx`; actions `notification_rules_load` / `notification_rules_save`, ADMIN_ONLY + `TAB_ACTIONS.automation`). Edited over time across 10+ areas. **NEW 2026-07-22 (migration `20260722130000`, +8 rows):** a new **`Tax Planners`** area with SIX planner bells — `TAX_planner_allocated`, `TAX_planner_returns_uploaded`, `TAX_planner_hlm_ready`, `TAX_planner_post_meeting`, `TAX_planner_decision2_ready`, `TAX_planner_impl_decision_ready` — all with `default_recipients '["ALLOCATED_TAX_PLANNER"]'` as a PLACEHOLDER (the real planner email is resolved per-call from the plan's `tax_planner_id` and passed directly by `notifyAllocatedPlanner`, `utils/tax-planner-notify.ts`); PLUS two `Tax`-area sweep tiers for the click-only Tax 4 Continue ladder — `TAX_postreview_continue_reminder_email` (email, delay 2) + `TAX_postreview_continue_stalled` (bell, delay 4). Prior newest: `TAX_returns_received` (area Uploads, FYI bell → Tracy/Tray, added 2026-07-15).
