...
| **"Reached out?" acknowledgements** (2026-08-12, v734) | | `sif_pf_ack_at`, `vote_r1_pf_ack_at`, `vote_r2_pf_ack_at`, `bg_choice_pf_ack_at`, `ddc_pf_ack_at`, `rev_share_final_pf_ack_at`, `agreement_sign_pf_ack_at`, `lic_payment_pf_ack_at` — **eight, one per stall ladder above** (the widest set of any pipeline). Records that a human chased the 96h escalation; **ONLY writer `automation_stall_ack`** (`pipeline:'specialist'`, `stall` = the prefix), un-ticking writes NULL. Nothing reads them — no sweep, no bell, no gate. The 48h/96h rows in each AI PC Admin block now render only when their own `_reminder_sent_at`/`_pf_notified_at` is stamped. **No backfill statement was written for this table** — all eight start NULL on every row. **The 96h recipient is hardcoded "Tracy" in the row label** (pre-existing sweep behaviour, unlike the other pipelines' "assigned PF"/"team member responsible"). Gotcha **#381**. |
