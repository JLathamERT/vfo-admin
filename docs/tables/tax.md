...
| `retainer_rev_paid` | text | `Yes` / `Failed` / `Money Mapping` / `N/A — No Share Due` / `Awaiting Connect Setup` (non-terminal, #303). **NULL is meaningful on a migrated row (2026-08-12, gotcha #379):** the Payment Continuation backfill normally pre-settles this to `N/A — No Share Due`, but with the **`retainer_share_unpaid`** checkbox it leaves the leg **NULL** so *Client decision 1* pays it for real. **`legacy_source='old-system'` AND `retainer_rev_paid IS NULL` is the signature of such a row**, and it is what every display rule must key on — testing `legacy_source` alone renders a settled story over live money (gotcha #378). |
...
| Column | Type | Notes |
|---|---|---|
| `assess_form` | jsonb | The submitted presentation details — ONLY `{ fee, taxes_without_plan, strategies:[{ name, invest_y1, invest_y2, gross_y1, gross_y2 }] }` (dollars, 2 dp; every field required on a kept row). **`taxes_without_plan` ("Total Taxes without a Tax Plan") was added 2026-08-06 and is REQUIRED**, validated FE/BE in lockstep with the identical message *"Total Taxes without a Tax Plan is required"*; it is the divisor the ROI deck's `PCT_REDUCED` and `TAXES_WITH` are built from (#336). Because `tax_save_assess_form` **rebuilds the whole jsonb every submit**, a **re-save is the only upgrade path** for rows submitted before that date — there is no backfill, and their absent key must read as "legacy, needs re-save", never as zero. Derived numbers (per-strategy totals, section totals, all of Net = Gross − Investment) are computed at render, NEVER stored — a consumer must compute them and must also handle the legacy `{question_1}` shape on pre-2026-07-29 rows (#306). |
...
**The table holds TWO person-types, split by `planner_role` (2026-07-27).** A **Team Member** logs into the SAME portal with the same capabilities, restrictions, allowlist and group-scope guards as a Tax Planner. **They ARE allocatable to a tax plan** (the old 400 was deleted 2026-08-10 — gotcha #294 as amended) and are the deliberate FIRST holder of a plan: a Team Member allocation raises the action-required "Allocate a Tax Planner for \<client\>" bell asking them to hand the plan to the planner who will run it. Because every planner bell and the whole planner payout leg key off `client_tax_plans.tax_planner_id` and nothing gates on the role, **a Team Member holding a plan DOES receive planner bells and IS the payout destination while they hold it** — which is why the hand-off is meant to happen long before any money leg resolves. Gotcha #294. **FRONTEND EXCEPTION as of 2026-08-13 (#390):** the Tax Priorities `stepGate` computes `taxPlannerAllocated` as `planner_role === 'Tax Planner'`, so while a **Team Member** holds the plan the *"Additional information required"* and *"Tax planner review complete"* steps stay **LOCKED** with the hint *"Allocate a Tax Planner (not a Team Member) first"*. **Backend capabilities are unchanged** — the allowlist, group-scope guards and every handler still treat the two roles identically. This is a UI guardrail only.
...
| `planner_role` | text | **NEW 2026-07-27** (`20260727120000_tax_planner_roles.sql`). not null, default `'Tax Planner'`; CHECK `tax_planners_planner_role_check` = `'Tax Planner' \| 'Team Member'`. **A `'Team Member'` IS allocatable** as of 2026-08-10 (the `tax_allocate_planner` 400 was deleted — #294 amended); `allocate-planner.ts` still READS the role, now to pick which hand-off bell to raise. The admin allocation dropdown and the portal's `group` roster **include** Team Members (each suffixed `— Team Member`); only the **KPI allocation leaderboard** still filters the role out. `save_tax_planner` sanitizes it and preserves the stored value when an edit omits it. Legacy rows read as `'Tax Planner'` via the default. Labelled **"Member Type"** in the admin UI. Gotcha #294. |
