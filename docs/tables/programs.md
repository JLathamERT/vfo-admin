...
| `video_url` | text | Optional training video — the member-side `VideoTask` embeds it. Two providers, detected by URL shape: a **YouTube** watch URL (`…?v=<id>`, played via the YT IFrame API) or a **Wistia** embed URL (`https://fast.wistia.net/embed/iframe/<mediaId>`, played in a plain iframe — any URL containing `wistia` takes this branch). Wistia `/s/<slug>` **share** links are NOT stored directly (can't be iframed); resolve them to the embed URL via oEmbed first — see gotcha #174. |
