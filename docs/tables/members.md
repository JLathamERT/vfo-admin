...
| `suspended` | boolean | default `false`. Status field — the admin's MANUAL toggle only. **No longer display-only (2026-08-24)** — see *Standing flags gate money* below. |
| `membership_suspended` | boolean | default `false`. Set/cleared ONLY by membership-billing automation (sweep + webhook); displays show "Suspended" when EITHER flag is true (gotcha #240). **Also gates payouts as of 2026-08-24** — see below. |
...
| `stripe_account_id` | text | **Stripe Connect ID** — used by `automation_CONTRACT_revshare` for Transfers. **Not a "has been set up" signal** — it is stamped the moment the Connect account is created, so a UI that branches on its presence shows a false "connected" (#318); ask `member_connect_status` instead. |
...
**Automation fields:** `stripe_account_id` (revshare), `suspended` / `membership_suspended` / `paused` (**revenue-share payout hold, 2026-08-24**), `ciq_enabled` (CIQ start-new gate) / `ciq_vfos_managed` (CIQ "Powered by VFO Services" label).

### Standing flags gate money as of 2026-08-24 *(v: 2026-08-24)*

**`suspended`, `membership_suspended` and `paused` used to be display-only — three booleans that every members-reading SURFACE had to OR together (#240) and nothing else read. That is no longer true.** Their first gating readers are the **four revenue-share payout engines**:

| Engine | Held value written |
|---|---|
| `actions/pipeline/contract-revshare.ts` (MAP 1) | `rec{N}_rev_paid` = `Held - Member Suspended` / `Held - Member Paused` |
| `actions/tax/revshare.ts` | `{retainer\|implementation}_rev_paid` = same two |
| `actions/msm/pip-revshare.ts` | `pip_rev_share_status` = same two |
| `utils/specialist-revenue-payout.ts` | `specialist_revenue_lines.payout_status` = `held_member_suspended` / `held_member_paused` (snake_case — that table's own vocabulary; **the Title-Case pair must not leak into it**) |

**Precedence — `utils/member-payout-hold.ts` `memberHoldReason(member)`:** `suspended || membership_suspended` ⇒ `"suspended"`; else `paused` ⇒ `"paused"`; else `null`. **Suspension outranks a pause**, and the automation flag is folded into "suspended" rather than getting a third state — so a member suspended for dues and a member suspended by an admin produce the identical held status and the identical internal notice. The **writer** rule of #240 is unchanged: automation must never write `suspended`, admin UIs must never write `membership_suspended`; only this READER is allowed to see all three at once.

**Two triggers release the hold** (both call `utils/member-payout-release.ts`): `member_profile_save` when a save takes the hold reason non-null → null (it re-reads the row after the upsert — a partial-profile save cannot be judged from the payload), and the membership sweep's pass-4 auto-unsuspend when clearing `membership_suspended` leaves no reason at all. Any new consumer that must not pay a member out of good standing calls `memberHoldReason` — do not re-derive the OR. **Selecting the three columns is now load-bearing in a payout handler's `members` select** (a missing column reads as `undefined` ⇒ not held ⇒ the money moves).

**Touched by:** `load_data`, `add_member`, `add_member_full`, `save_member`, `delete_member`, `member_profile_load`, `member_profile_save` (incl. `introduced_by_member_number` + `connection_type` — a full passthrough spread; **and since 2026-08-24 the hold-release trigger above, whose response gains an additive `payout_release` key when it fires**), `upload_headshot` (profile headshot → `headshots` bucket), `automation_CONTRACT_revshare`, `automation_TAX_revshare`, `automation_PIP_revshare`, `specialist_revenue_payout`, `automation_MEMBERSHIP_sweep`. Frontend: [MembersPanel.jsx](src/components/admin/MembersPanel.jsx), [MemberPortal.jsx](src/pages/MemberPortal.jsx).
