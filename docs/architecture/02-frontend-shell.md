...
- **Advisor Growth Plan** (in progress, **advisor-only**) → the admin member-detail view (advisors, not accountants) has a new `Growth Plan` `FeatureTabDropdown` (left of CIQ, gated by a `growthPlan` prop on `MemberDirectoryView`) → [`AdminGrowthPlan`](src/components/growth/AdminGrowthPlan.jsx) routing the `gp_*` sub-tabs in `src/components/growth/*` (G1 scoring → G5 one-page value/effort matrix). Calls `growth_plan_load` / `_load_history` / `_save_score` / `_save_summary` / `_save_actions` / `_save_accountability`. The member-portal `growthplan` tab is still `ComingSoon` (Phase 8). See [../GROWTH_PLAN_HANDOFF.md](../GROWTH_PLAN_HANDOFF.md)
