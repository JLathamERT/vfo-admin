...
> **Inconsistency:** The card-fee math in [PayPage.jsx:76](src/pages/PayPage.jsx) — `(payment_amount + payment_amount * 0.029 + 0.30).toFixed(2)` — is the *naive* fee (added on top), but the server-side card calculation in [vfo-admin-api/index.ts:1116](C:/vfo-edge-functions/supabase/functions/vfo-admin-api/index.ts) uses the gross-up formula `Math.round((baseAmount + 0.30) / (1 - 0.029) * 100)`. The two differ by a few cents on the fee component. The displayed amount is informational; the actual Stripe charge uses the gross-up formula.
