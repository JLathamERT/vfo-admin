...
- **Advisor Growth Plan** (**advisor-only**; Phases 1–8 + custom/sub-tasks) → the admin member-detail view (advisors, not accountants) has a `Growth Plan` `FeatureTabDropdown` (left of CIQ, gated by a `growthPlan` prop on `MemberDirectoryView`) → [`AdminGrowthPlan`](src/components/growth/AdminGrowthPlan.jsx) routing the `gp_*` sub-tabs in `src/components/growth/*` (G1 scoring → G5 one-page matrix + Accountability Mode + Parking Garage + Growth History). Calls `growth_plan_load`/`_load_history`/`_load_admins`/`_load_audit`/`_save_score`/`_save_summary`/`_save_actions`/`_save_accountability`/`_set_accountability`/`_add_action`/`_delete_action`. The member-portal `growthplan` tab is now live ([`MemberGrowthPlan`](src/components/member/MemberGrowthPlan.jsx), advisor-gated). See [../GROWTH_PLAN_HANDOFF.md](../GROWTH_PLAN_HANDOFF.md)
...
- `EmailTemplatesPanel` → `automation_load_email_templates`, `automation_save_email_template` ([EmailTemplatesPanel.jsx:18, 40](src/components/admin/EmailTemplatesPanel.jsx)) — the `TEAM` pipeline tab saves a `to_list` array (not `cc_list`/`bcc_list`); see the onboarding Team Member subsection below
...
| `growthplan` | Growth Plan | [MemberGrowthPlan](src/components/member/MemberGrowthPlan.jsx) — **advisor-gated** (`member_category==='advisor'`, else `ComingSoon`) | `growth_plan_load` (self-scoped); under Accountability Mode `growth_plan_save_accountability` / `_save_actions` / `_add_action`. Two sub-tabs: One Page Plan (read-only; progress + per-row "+ Add sub-task" editable under accountability) + Add a Priority (preset list + "Create your own") |
