...
| `/admin` | [AdminPortal](src/pages/AdminPortal.jsx) | admin session | Top-level admin shell — Advisors / Accountants / Strategic Members / Specialists / Automation / Accounting tabs (Automation + Accounting are Jake-only; Payments is now a sub-tab of Accounting alongside VFO Specialist Payment Input + VFO Specialist Revenue) |
...
| `/specialist-revenue-pay` | [SpecialistRevenuePayPage](src/pages/SpecialistRevenuePayPage.jsx) | URL token | PUBLIC page from the VFO Specialist Revenue "payment request" email, used for the **ACH-debit-link** method only. **ACH-only (2026-06-30):** no method choice — on load it auto-redirects to the Stripe ACH checkout (`specialist_revenue_pay_load` + `specialist_revenue_checkout`, `metadata.pipeline=VFO_SPECIALIST_REVENUE`). Added 2026-06-29. **NOTE (2026-07-06):** the New-Payment form now offers a per-request method pick — ACH-debit link (this page) OR **Bank transfer**; bank-transfer requests skip this page entirely (the specialist gets virtual bank details in the email and pushes funds from their own bank), and `pay_load`/`checkout` **refuse** bank-transfer tokens (400). |
