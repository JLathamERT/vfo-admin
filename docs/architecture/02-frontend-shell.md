...
| `/admin` | [AdminPortal](src/pages/AdminPortal.jsx) | admin session | Top-level admin shell — Advisors / Accountants / Strategic Members / Specialists / Automation / Accounting tabs (Automation + Accounting are Jake-only; Payments is now a sub-tab of Accounting alongside VFO Specialist Payment Input + VFO Specialist Revenue). Accounting → Specialists also gained **"VFO Specialist Recurring Revenue Payments"** (nav key `specialist_recurring`, superadmin-gated, 2026-07-09) mounting [SpecialistRecurringPanel.jsx](src/components/admin/SpecialistRecurringPanel.jsx) — lists all recurring plans (via `specialist_revenue_recurring_load`) with per-plan Cancel (confirm → `specialist_revenue_recurring_cancel`) |
...
| `/membership-pay` | [MembershipPayPage](src/pages/MembershipPayPage.jsx) | URL token | PUBLIC page from the Member Membership Fees setup emails (added 2026-07-13, dev-only until the next `npm run deploy`). Member picks Bank (ACH) or Card and PAYS THEIR FIRST membership payment at a `mode=payment` Stripe Checkout (`membership_setup_load` / `membership_setup_checkout`; method saved via `setup_future_usage=off_session`; Legacy = no card fee, New Model = 2.9%+$0.30 gross-up). Save-only `mode=setup` variants: annual transfers, fully-credited $0 plans, and ACTIVE plans — where the same link becomes the "Update Your Payment Method" page (the failed-payment email button). States: pay-today / update / success / canceled / invalid. See [flows/membership-fees.md](../flows/membership-fees.md). |
...
- `MembersPanel` → `add_member_full`, `save_member`, `delete_member`, `member_profile_load/save`, `gc_*`, `member_program_notes_*`, `load_member_login`, `create/update_member_login` ([MembersPanel.jsx](src/components/admin/MembersPanel.jsx))
...
- New component [`components/admin/NewModelSaleModal.jsx`](src/components/admin/NewModelSaleModal.jsx): an in-panel modal that opens on the Stage-3 "Create Advisor/Accountant" button in [`AdvisorOnboarding.jsx`](src/components/admin/AdvisorOnboarding.jsx) / [`AccountantOnboarding.jsx`](src/components/admin/AccountantOnboarding.jsx). Collects three dropdowns (closer / setter / introduced-by — all default `n/a`), optional **Company name** + **Website** text inputs, and a **member-introduction** typeahead (searches `member_category` advisor/accountant from the page's cached `load_data`). On confirm it hands those `sale_*` fields to the existing create-member action (`automation_ADVISOR_createadvisor` / `automation_ACCOUNTANT_createaccountant`), which now also drafts the team-notification email. **Exports `SALES_TEAM_NAMES`** (Rachael Hopson, Ian Welham, Anton Anderson, Paul Latham, Seth Hartford) — the shared dropdown list, also imported by both onboarding components for the Stage-1 step below. (The dropdown names are display-only; the email *recipients* are the separate `to_list` editor in Email Templates.)
