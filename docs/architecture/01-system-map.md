...
- **Fourteen background jobs via `pg_cron` + `pg_net.http_post`** (staggered to avoid races on the same row; jobids 2–15. Also live: `regular-map4-followup-sweep-daily` jobid 11 @09:30 UTC, `growth-overdue-sweep-daily` jobid 12 @10:00 UTC, and — added 2026-07-10 — `reminder-sweep-5min` jobid 14 (`*/5 * * * *`, `automation_REMINDER_sweep`: delivers due `personal_reminders` as bell notifications; the only non-daily job) and `notifications-purge-daily` jobid 15 @10:30 UTC (`automation_NOTIFICATIONS_purge`: hard-deletes READ notifications older than 90 days). Setup docs: `supabase/cron/reminder-sweep.sql` + `notifications-purge.sql`):
