...
| `vfo-admin-api/router/dispatch.ts` | ~676 lines | The two action-dispatch tables: `PUBLIC_HANDLERS` (106 entries) + `AUTH_HANDLERS` (189 entries). Each entry adapts a handler from `actions/<group>/<file>.ts` to a uniform ctx-based signature. |
