...
| `vfo-admin-api/middleware/auth.ts` | ~130 lines | The token gate. `authenticate(action, body, supabase, json)` validates `body.token` against `admin_sessions`, then applies (in order) the client + specialist deny-by-default gates, the `SUPERADMIN_ONLY_ACTIONS` gate (NEW 2026-06-16 — Jake-only, 12 Automation-panel actions), `ADMIN_ONLY_ACTIONS`, and `MEMBER_SCOPED_ACTIONS` from `constants/role-gates.ts`. |
