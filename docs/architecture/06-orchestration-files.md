...
| `vfo-admin-api/router/dispatch.ts` | ~681 lines | The two action-dispatch tables: `PUBLIC_HANDLERS` (100 entries) + `AUTH_HANDLERS` (198 entries). Each entry adapts a handler from `actions/<group>/<file>.ts` to a uniform ctx-based signature. (2026-06-16: 6 Tax actions moved PUBLIC→AUTH.) |
...
| `vfo-admin-api/middleware/auth.ts` | ~130 lines | The token gate. `authenticate(action, body, supabase, json)` validates `body.token` against `admin_sessions`, then applies (in order) the client + specialist deny-by-default gates, the `SUPERADMIN_ONLY_ACTIONS` gate (NEW 2026-06-16 — Jake-only, 13 actions: 12 Automation-panel + the global Payments page `all_payments_load`), `ADMIN_ONLY_ACTIONS`, and `MEMBER_SCOPED_ACTIONS` from `constants/role-gates.ts`. |
| `vfo-admin-api/actions/<group>/*.ts` | ~300 files | One file per action handler. Groups: `auth/`, `data/`, `client-notes/`, `member-program-notes/`, `email-templates/`, `notifications/`, `admins/`, `member-logins/`, `vault/`, `members/`, `specialists/`, `gc/`, `coaching/`, `tax/` (~28 files), `ciq/`, `onboarding/`, `msm/` (~45 files incl. PIP), `pipeline/` (17 files), `regular/`, `pft/`, `advisor/` (22 files incl. `save-team-member.ts`), `accountant/` (23 files incl. `save-partnership.ts` + `save-team-member.ts`). |
| `vfo-admin-api/utils/`, `constants/`, `types/`, `integrations/` | ~26+ files (19 utils + 7 constants + types + integrations) | Shared helpers: `cors.ts`, `crypto.ts`, `format-date.ts`, `html-templates.ts`, `json.ts`, `pf-emails.ts`, `member-number.ts` (category-relative `nextMemberNumber`), `new-model-sale-email.ts` (`sendNewModelSaleEmail` — internal "New Model Sale" team draft on advisor/accountant create), role-gate arrays, `constants/onboarding-team.ts` (`teamMemberRecipient` — onboarding team-member name → @elitert login for notification routing), `JsonResponder` / `AuthContext` / ctx types, integration scaffolding for Stripe/BoldSign/Google. |
