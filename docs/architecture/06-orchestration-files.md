...
| `vfo-admin-api/utils/`, `constants/`, `types/`, `integrations/` | ~16 files | Shared helpers: `cors.ts`, `crypto.ts`, `format-date.ts`, `html-templates.ts`, `json.ts`, `pf-emails.ts`, `member-number.ts` (category-relative `nextMemberNumber`), role-gate arrays, `JsonResponder` / `AuthContext` / ctx types, integration scaffolding for Stripe/BoldSign/Google. |
