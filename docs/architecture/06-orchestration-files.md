...
| `vfo-admin-api/router/dispatch.ts` | ~700 lines | The two action-dispatch tables: `PUBLIC_HANDLERS` (**129 entries**) + `AUTH_HANDLERS` (**312 entries**) = 441 dispatched, +6 logins in `index.ts` = the 447 live count. _(Counts re-verified 2026-07-30 by `awk` over each table block; the "102 + 200" this line asserted had gone stale by many sessions — re-count, never trust it.)_ Each entry adapts a handler from `actions/<group>/<file>.ts` to a uniform ctx-based signature. (2026-06-16: 6 Tax actions moved PUBLIC→AUTH; +3 Phase D card-change regs — `payments_send_card_update` AUTH/superadmin, `payments_loadcardupdate` + `payments_cardupdate_checkout` PUBLIC.) |
