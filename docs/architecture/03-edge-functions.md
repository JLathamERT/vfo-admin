...
- `presentation-templates` (**PRIVATE**, 50 MB, added 2026-08-06) — read by `actions/tax/generate-presentation.ts` (`tax_generate_presentation`). Holds exactly ONE object, **`ROI-template-master.pptx`** (~27.6 MB), the tokenized master ROI deck. **Created with NO `storage.objects` policies at all** — service-role only — and there is **no upload path in code**: the master is replaced by a human through the Supabase Dashboard, under that exact object name, after being rebuilt by `scripts/roi-presentation/build_master.py` in this repo (#336). The filled deck is never written back to Storage; it goes straight to Google Drive.
