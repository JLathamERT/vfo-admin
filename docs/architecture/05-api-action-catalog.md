...
All **177 actions** dispatched by `vfo-admin-api` (3 logins + 43 PUBLIC + 131 AUTH). Composition: MAP1 baseline of 134 + ~28 tax handlers in `actions/tax/` (see [../flows/tax-planning.md](../flows/tax-planning.md)) + 15 Advisor Onboarding handlers in `actions/advisor/` (Phases 1-4 deployed 2026-05-22 through 2026-05-26; see `ADVISOR_ONBOARDING_RESUMPTION.md` at repo root). The post-refactor catalog cites file paths (not line numbers — line numbers shift across handler edits). Action names and behavior are byte-equivalent to v194 except for explicit follow-up changes noted in entries below.
