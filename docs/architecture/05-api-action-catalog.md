...
> **Pre-refactor count.** v194 had 130 dispatch references. Phase 6 mechanical removed two: the duplicate `msm_update_client` registration (always unreachable) and the dead `automation_CONTRACT_stripewebhook` handler (doubly unreachable). After that the total was 128. Subsequent feature/refactor work added the remaining handlers (revshare sweep, sandbox-config save, chargescheduled sweep, check-payment flow, the entire `actions/tax/` module, the Tax 4/5 redesign and Tax Planning program_id=4 alignment, 21 handlers across the new `actions/advisor/` module for Advisor Onboarding Phases 1-6 + login setup + admin automation loader, 14 handlers for the PIP Meetings purchase chain, and 22 handlers across the new `actions/accountant/` module for Accountant Onboarding + Partnership? step). **Current count is 3 logins + 71 PUBLIC_HANDLERS + 149 AUTH_HANDLERS = 223** — verify any time via the parity gate (`logins + dispatch == 223`).
...
22 handlers in `actions/accountant/*.ts` (cloned from advisor with `save-partnership.ts` added). Token-swap convention: every advisor identifier → accountant equivalent (e.g. `automation_ADVISOR_decision` → `automation_ACCOUNTANT_decision`, table `advisor_onboarding` → `accountant_onboarding`, column `agreement_signed_by_advisor_at` → `agreement_signed_by_accountant_at`). 14 PUBLIC chain-callable + 7 AUTH (admin-triggered) + 1 token-page chain (`_clientdecision`). Trigger graph mirrors advisor exactly with one inserted step (`save_accountant_partnership`) between Stage 1 Prelim Meeting and Stage 1 Decision. Stripe metadata convention: `pipeline='ACCOUNTANT_ONBOARDING'`, `payment_kind='onboarding'`. Payment amount is DYNAMIC ($2,000 / $2,600 / $4,000 / $4,600) based on the partnership choice + corporate add-on; computed in `ceo-countersign.ts` and persisted to `accountant_onboarding.payment_amount`. Same `setup_future_usage=off_session` for 6-month renewal review. RLS now enabled on both `advisor_onboarding` and `accountant_onboarding`.
...
Plus 14 more PUBLIC handlers that are direct functional clones (different token names + tables, same shape): `_declineemail`, `_clientdecision`, `_stripecustomer`, `_paymentemail`, `_loadpayment`, `_stripecheckout`, `_confirmationemail`, `_loginsetupemail`, `_loadloginsetup`, `_submitloginsetup`. Plus 6 AUTH handlers: `load_accountant_onboardings`, `load_accountant_onboarding`, `create_accountant_onboarding`, `save_accountant_prelim_meeting`, `automation_ACCOUNTANT_decision`, `save_accountant_partnership`.
...
| `automation_ADVISOR_createmember` | `actions/advisor/create-member.ts` | `advisor_onboarding`, `members` + `member_number_baselines` (via `nextMemberNumber`) | `member_plugin_settings`, `members` (`member_type='Implementation'`, `elite_status='Active'`, `advisor_model='New Model'`, `member_category='advisor'`, `revenue_decision='Money Mapping'`, `onboarding_id=<advisor_onboarding.id>` FK), `advisor_onboarding` (`member_number`, `member_created_at`, `revenue_decision='Money Mapping'`, `login_setup_token`, `login_setup_token_expires_at`) | Fired from Advisor Onboarding Stage 3 "Create Advisor & Send Setup Link" button. Member number via category-relative `nextMemberNumber('advisor','New Model')` (gotcha #48 — no fixed 60100 start). Idempotent on `advisor_onboarding.member_number` already set. No Stripe Connect — Money Mapping only. After member create succeeds (or on idempotent retry), generates a 14-day single-use `login_setup_token` and chains `automation_ADVISOR_loginsetupemail` via service-role to draft the passcode-setup email. |
