...
| `automation_CONTRACT_confirmationemail` | `actions/pipeline/contract-confirmation-email.ts` | `pmap1`, `clients`, `members`, `psbx_cfg`, `email_templates` | `pmap1`(confirmation_status='Sent' — payment-1 only) | Gmail draft to client. Accepts an optional `payment_number` (defaults 1) so MAP 1 installments 2-4 get a confirmation (chained per-installment from `_chargescheduled_sweep`); the payment-1-only guard + "made their first payment" PF notification + `confirmation_status` write are preserved for N=1. |
...
23 handlers in `actions/accountant/*.ts` (cloned from advisor with `save-partnership.ts` + `save-team-member.ts` added). Token-swap convention: every advisor identifier → accountant equivalent (e.g. `automation_ADVISOR_decision` → `automation_ACCOUNTANT_decision`, table `advisor_onboarding` → `accountant_onboarding`, column `agreement_signed_by_advisor_at` → `agreement_signed_by_accountant_at`). 14 PUBLIC chain-callable + 8 AUTH (admin-triggered) + 1 token-page chain (`_clientdecision`). Trigger graph mirrors advisor exactly with one inserted step (`save_accountant_partnership`) between Stage 1 Prelim Meeting and Stage 1 Decision. Stripe metadata convention: `pipeline='ACCOUNTANT_ONBOARDING'`, `payment_kind='onboarding'`. Payment amount is DYNAMIC ($2,000 / $2,600 / $4,000 / $4,600) based on the partnership choice + corporate add-on; computed in `ceo-countersign.ts` and persisted to `accountant_onboarding.payment_amount`. Same `setup_future_usage=off_session` for 6-month renewal review. RLS now enabled on both `advisor_onboarding` and `accountant_onboarding`.

Diffs from advisor:

- **`save_accountant_partnership`** (AUTH) — new handler with no advisor analog. Body `{ onboarding_id, partnership: 'No accountant partnership' | 'Accountant Partnership' | null }`. Writes `accountant_partnership` + `accountant_partnership_at` on the row. Frontend gates Stage 1 Decision buttons until the partnership choice is saved.
...
Plus 14 more PUBLIC handlers that are direct functional clones (different token names + tables, same shape): `_declineemail`, `_clientdecision`, `_stripecustomer`, `_paymentemail`, `_loadpayment`, `_stripecheckout`, `_confirmationemail`, `_loginsetupemail`, `_loadloginsetup`, `_submitloginsetup`. Plus 7 AUTH handlers: `load_accountant_onboardings`, `load_accountant_onboarding`, `create_accountant_onboarding`, `save_accountant_prelim_meeting`, `automation_ACCOUNTANT_decision`, `save_accountant_partnership`, `save_accountant_team_member`.
...
Two new AUTH/admin handlers record which team member "owns" an onboarding (the Stage-1 "Team Member Responsible" step), so its stall/decision/ready notifications route to that person's bell via `teamMemberRecipient()` (`constants/onboarding-team.ts`; 5 names → `@elitert.com` logins). Both are in `ADMIN_ONLY_ACTIONS`.
...
- `automation_ADVISOR_clientdecision` / `automation_ACCOUNTANT_clientdecision` — the clicked-Yes notification routes to `teamMemberRecipient()`; the advisor "No" notification link is fixed from `/admin` to the `/admin?tab=advisors&section=advisor_onboarding&onboarding=<id>` deep-link.
- `automation_ADVISOR_createmember` / `automation_ACCOUNTANT_createmember` (Members table below) — now accept 7 `sale_*` body fields (`sale_closer`, `sale_setter`, `sale_introduced_by`, `sale_introduced_member_number`, `sale_introduced_member_name`, `sale_company_name`, `sale_website`), persist them on the onboarding row, fire `sendNewModelSaleEmail()` in BOTH the happy + idempotent-retry paths, and clear the "Ready to create" action-required notification.
...
| `automation_REGULAR_submitmap4form` | `actions/regular/submit-map4-form.ts` | `client_priority_tracks` | `client_priority_tracks`(map4_form_data, map4_form_submitted_at); `notifications`(Tracy+Tray FYI) | PUBLIC token page. Saves answers + a dismissible FYI to `tnmiller@`/`tvaldes@` linking to the client's Regular Priorities tab. |
...
| `automation_TAX_charge_implementation` (PUBLIC) | `actions/tax/charge-implementation.ts` | `client_tax_plans`, `clients`, `psbx_cfg`(TAX) | `client_tax_plans`(`implementation_charge_status`, `implementation_payment_intent_id`, `implementation_charge_date`, `checkout_token` regenerated on failure) | Payment method: prefers `client_tax_plans.default_payment_method_id`, else expands the PM off the retainer PI (Stripe `GET /v1/payment_intents/{retainer_payment_intent_id}?expand[]=payment_method`). Stripe `POST /v1/payment_intents` (off-session, confirm=true; card grossed up, ACH at base; idempotency key `tax-impl-{plan}-{pm_or_pi_suffix}-{date}`). On failure → fresh checkout_token, admin notification, Gmail draft to client with `/tax-pay` link. Called from sweep (24h Proceed lock) and from `automation_TAX_implementfinaldecision` (Undecided→Proceed). |
