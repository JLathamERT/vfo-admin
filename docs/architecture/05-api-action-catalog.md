...
All **218 actions** dispatched by `vfo-admin-api` (3 logins + 68 PUBLIC + 147 AUTH). Composition: MAP1 baseline of 134 + ~28 tax handlers in `actions/tax/` (see [../flows/tax-planning.md](../flows/tax-planning.md)) + 21 Advisor Onboarding handlers in `actions/advisor/` (Phases 1-6 deployed 2026-05-22 through 2026-05-26 + member-portal login setup chain + admin Automation Panel loader added 2026-05-28; see `ADVISOR_ONBOARDING_RESUMPTION.md` at repo root) + 14 PIP Meetings handlers (deployed 2026-05-27; see [../flows/pip-meetings.md](../flows/pip-meetings.md)) + 22 Accountant Onboarding handlers in `actions/accountant/` (cloned from advisor with `save-partnership` added for the new Stage 1 Partnership? step; deployed 2026-05-28 as v323→v330; see `ACCOUNTANT_ONBOARDING_RESUMPTION.md` at repo root). The post-refactor catalog cites file paths (not line numbers — line numbers shift across handler edits). Action names and behavior are byte-equivalent to v194 except for explicit follow-up changes noted in entries below.
...
- **`automation_ACCOUNTANT_createmember`** — does NOT write `revenue_decision` on the inserted `members` row (accountants don't have one). Member number namespace `[30000, 90000)` for New Model (the default) — filters to `members WHERE accountant_onboarding_id IS NOT NULL OR (onboarding_id IS NULL AND member_type IN ACCOUNTANT_TYPES)` so legacy 59xxx advisor numbers don't pollute. Writes `members.accountant_onboarding_id` FK + `advisor_model='New Model'`. The manual Add Accountant form additionally supports Legacy Model accountants in the `[90000, ∞)` namespace.
