...
All **243 actions** dispatched by `vfo-admin-api` (3 logins + 240 dispatched; PUBLIC/AUTH split tracked in `SESSION_REFERENCE.md` LIVE STATE). Composition (approximate, historical): MAP1 baseline of 134 + ~28 tax handlers in `actions/tax/` (see [../flows/tax-planning.md](../flows/tax-planning.md)) + 21 Advisor Onboarding handlers in `actions/advisor/` (see `ADVISOR_ONBOARDING_RESUMPTION.md` at repo root) + 14 PIP Meetings handlers (see [../flows/pip-meetings.md](../flows/pip-meetings.md)) + 22 Accountant Onboarding handlers in `actions/accountant/` (see `ACCOUNTANT_ONBOARDING_RESUMPTION.md` at repo root) + **~25 Specialist Onboarding handlers** in `actions/onboarding/` (Stages 1–3 automation incl. the 2026-06-04 DD Checklist + Step 3 receipt-email revamp — `loadddc`/`saveddc`/`ddcuploadurl`/`submitddc`/`ddchelp`/`revsharefinal`/`ddcapprove`/`ddcedits`/`revsharefinalize`/`ddcdownload`; the old `revsharedecide` was removed; deployed v385; see [../flows/specialist-onboarding.md](../flows/specialist-onboarding.md)). ⚠️ Live v385 still registers the removed `revsharedecide` (244) until the next deploy → 243. The catalog cites file paths (not line numbers). Action names/behavior are byte-equivalent to v194 except explicit follow-ups noted in entries below.
