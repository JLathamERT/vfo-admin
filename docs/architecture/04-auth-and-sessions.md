...
| Gate | File | Behavior |
|---|---|---|
| `ADMIN_ONLY_ACTIONS` array | `constants/role-gates.ts` | Non-admin callers get `403 Forbidden`. Includes the client vault admin actions (`vault_tax_admin_upload_url`, `vault_tax_list`, `vault_tax_download`, `vault_tax_delete`, `vault_gen_list`, `vault_gen_upload_url`, `vault_gen_download`, `vault_gen_delete`), and — **added 2026-06-10** — the specialist vault add/remove handlers `specialist_vault_admin_upload_url` + `specialist_vault_admin_delete` (alongside the existing `specialist_vault_admin_list`/`_admin_download`). |
