...
| `resolveCallerPlanner(supabase, body)` | (helper, NEW 2026-08-13) Resolves the caller to `{ id, name }` as a tax planner / Team Member, or **null for admins and everyone else**. Walks the SAME path as `resolvePlannerGroupIds` — `body.token` → `admin_sessions.email` → **`allowed_admins` precedence check** → `tax_planner_logins` → `tax_planners` — so an admin who ALSO holds a planner login is never typed as a planner, matching `middleware/auth.ts`. **This is the first consumer of that path that is NOT a 403 gate:** it exists for notification attribution only (`utils/vault-planner-notify.ts`) and must never be used as an access control. |
...
> ### ⚠️ The precedence chain is a GENERAL collision, not two special cases (2026-08-20 — gotcha #425, **found, NOT fixed**)
>
> The two caveats above are instances of one structural fact: **`admin_sessions` stores no `login_type`.** It holds the token, the email and an expiry, so `middleware/auth.ts` re-derives the caller's role at request time by probing the login tables in a fixed order — **admin → client → specialist → tax_planner → member** — and returning on the first hit. **Which portal actually minted the session is not recorded and cannot be recovered.**
>
> Therefore **any** email that holds two portal identities resolves to the higher-precedence one, in both directions, for every session. Worked example: Carson Grover (`cgrover@wealthig.com`) is **tax planner id 3** and, from 2026-08-20 18:41Z, **member 58014**. His member session resolved as `callerRole "tax_planner"`, so the deny-by-default gate matched his member-portal actions against `TAX_PLANNER_ALLOWED_ACTIONS` and **403'd every one of them** — a valid, unexpired session rendering blank tabs and a *"Coming soon"* Growth Plan (`function_edge_logs`: three 403s per tab click, 18:41:44 through 18:45). The reverse case is an authorization **widening**: creating a tax-planner row on an email that already has a member login silently promotes that person's existing member sessions.
>
> **Rule until this is fixed: an email address may hold at most ONE portal identity. Before creating any login row, check the other five login tables for that address.** The queued fix — **flagged, not built** — is a `login_type` column on `admin_sessions`, stamped by all six login actions and read here in place of the email probe, with the probe retained as the fallback for pre-existing sessions. Note also that **`src/lib/api.js` redirects on a 401 but swallows a 403** (gotcha #426), which is why this presents as an empty page rather than an error.
