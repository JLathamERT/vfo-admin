...
**OPEN / OWED**
- **Test Client 62 fixtures — DB re-verified, two file/deck remainders owed.** Plans: **0** (145/146 stayed deleted). The two permanently-stuck bells — Tray's HLM-confirmation and the test planner's decision-2 — are **confirmed GONE** (SQL: no `%HLM%`, no `Complete Client decision 2%`, and no `Download the ROI presentation%` row for client 62). The client is NOT at zero notifications: **4 rows**, and two of them are cron-minted, not fixtures — 1347/1348 *"Test Client hasn't set up their payment method"* (both `2026-08-19 04:00Z`, dismissible, **one still unread**) prove a live sweep keeps re-minting on this client after any wipe, so "clean" here means *no plan rows*, never *no bells*. 1359/1360 are this session's vault-drop test bells, both marked read. **Owed by hand:** the **TEN** decks in the Drive *"ROI Presentations"* folder (`ROI Presentation - Test Client - 2026-08-17/18`; a Drive MCP attempt 404'd — they belong to the backend's service account), and the test file **`ERT_Zoom_Background.png`** in client 62's **Sensitive Documents** vault, which must be deleted through the **UI/Storage API** — SQL cannot touch it (#206). *(v: 2026-08-19)*
