...
#    Plus helper draft-agreement-pdfs v1 (no business logic). (v: 2026-08-18 — v759 / v40)
...
cd C:\vfo-react;          git tag -l 'live-*'         --sort=v:refname | Select-Object -Last 1   # (v: 2026-08-18 → live-153-client-overview-owners)
cd C:\vfo-edge-functions; git tag -l 'backend-good-*' --sort=v:refname | Select-Object -Last 1   # (v: 2026-08-18 → backend-good-2026-08-18-v758)

# 3. Action-count parity — the ANCHORED patterns are required; a raw unanchored
#    grep on index.ts returns 7 (the 7th is a comment on line 3). (v: 2026-08-18 → 6 + 457 = 463)
...
**OPEN / OWED**
- **Test fixtures on Test Client 62 — re-derived AGAIN, do not carry the older wording:** plans **136 and 138 are GONE** (discharged). Live now: plan **145** (program 4, `status='stopped'`) + its **4** `client_tax_progress` rows + **one** `client_tax_specialists` row (Charles Riggs, `expert_id` 3553), and the 2026-08-18 ROI fixture plan **146** (program 4, `status='live'`, assess form submitted + deck generated) + its **5** progress rows. **64** TAX notifications on the client, of which **3** are unread action-required: two are **permanently stuck** because their plans were deleted (*"Send the detailed tax plan meeting confirmation email…"* → Tray, *"Complete Client decision 2…"* → the test planner — the user was asked about deleting them and has not answered), the third (*"Download the ROI presentation for Test Client"*) belongs to plan 146 and clears through the normal flow. **TEN** generated decks in the Drive *"ROI Presentations"* folder (**7** dated 2026-08-17, **3** dated 2026-08-18) must be removed **by hand**; the DB rows are a scripted delete. *(v: 2026-08-18 — every figure re-queried this session)*
...
- **`vfo-admin-api`** — `index.ts` orchestrator + ~470 modular handler files in `actions/<group>/*.ts` + `router/dispatch.ts` + `router/webhooks.ts` + `middleware/auth.ts` + `utils/` + `constants/` + `integrations/`. Serves all 463 actions. *(v: 2026-08-18)*
