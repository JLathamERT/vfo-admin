...
**OPEN / OWED**
- **Test Client 62 DB fixtures are CLEAN as of 2026-08-18 (later that day):** on the user's instruction, plans **145 and 146** were deleted (9 progress rows + 1 specialist row cascaded) and **ALL 71 notifications** on the client (64 TAX incl. the two permanently-stuck bells, 7 VAULT) were deleted — verified 0/0/0 afterward. The only remainder: **TEN** generated decks in the Drive *"ROI Presentations"* folder (`ROI Presentation - Test Client - 2026-08-17/18`) still need **hand-deletion** — a Drive MCP attempt 404'd (wrong Google context; the decks belong to the backend's service account). *(v: 2026-08-18)*
