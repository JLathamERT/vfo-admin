...
#    Plus helper draft-agreement-pdfs v1 (no business logic). (v: 2026-08-20 — v766 / v40)
...
cd C:\vfo-react;          git tag -l 'live-*'         --sort=v:refname | Select-Object -Last 1   # (v: 2026-08-20 → live-157-gc-allocation)
cd C:\vfo-edge-functions; git tag -l 'backend-good-*' --sort=v:refname | Select-Object -Last 1   # (v: 2026-08-20 → backend-good-2026-08-20-v764)
...
**OPEN / OWED**
- **The Growth Credit allocation backend is LIVE (v764) while the frontend is NOT published — and in that window a save from the OLD panel WIPES an allocation.** `gc_manage_service`'s UPDATE writes `allocated_admin_email` + `scheduling_link` on **every** save and a body omitting them cleans to `null`; the published panel does not send them. Any admin saving a service from the live Automation & Config page before `npm run deploy` silently clears that row's team member and link, and the seeds must be re-run. **The new panel always sends current values, so publishing the frontend closes this** — until then it is the most urgent item on this list. **(closing: `npm run deploy` pending in this session's Part 4 — delete this whole line at the 4D re-stamp, not before.)** *(v: 2026-08-20)*
