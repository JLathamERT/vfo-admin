...
- **SHIPPED 2026-08-20 (v772) — sessions now record the portal that minted them (#425/#426); the "one email = one portal identity" rule is RETIRED.** `admin_sessions.login_type` is stamped by all six login actions and resolves the role (re-verified; row gone ⇒ 401), email precedence kept ONLY as the fallback for **legacy NULL sessions, which expire ~8h after the deploy**. Verified live: Carson both directions, a 5-portal click-through, smoke 5/5. **FE banner fix (all four remaining portal shells) is on the branch — deploy pending.** Last unexercised path: **Carson's own organic re-login** — his broken pre-deploy sessions expire **2026-08-21 ~02:40Z**. *(v: 2026-08-20)*
