...
#    Plus helper draft-agreement-pdfs v1 (no business logic). (v: 2026-08-20 — v771 / v40)
...
cd C:\vfo-react;          git tag -l 'live-*'         --sort=v:refname | Select-Object -Last 1   # (v: 2026-08-20 → live-159-additional-contacts)
cd C:\vfo-edge-functions; git tag -l 'backend-good-*' --sort=v:refname | Select-Object -Last 1   # (v: 2026-08-20 → backend-good-2026-08-20-v771)

# 3. Action-count parity — the ANCHORED patterns are required; a raw unanchored
#    grep on index.ts returns 7 (the 7th is a comment on line 3). (v: 2026-08-20 → 6 + 459 = 465)
...
- **`vfo-admin-api`** — `index.ts` orchestrator + ~470 modular handler files in `actions/<group>/*.ts` + `router/dispatch.ts` + `router/webhooks.ts` + `middleware/auth.ts` + `utils/` + `constants/` + `integrations/`. Serves all 465 actions. *(v: 2026-08-20)*
...
- **#425 / #426** — **one email = one portal identity.** `admin_sessions` records no `login_type`, so the role is re-derived from the EMAIL with precedence admin → client → specialist → tax_planner → member; a second identity on the same address silently shadows the lower one — a blank portal one way, an authorization WIDENING the other. Check the other five login tables before creating any login row. It presents as an unbuilt feature because `api.js` redirects on 401 but **swallows 403**: when a surface is blank and the data exists, read `function_edge_logs` for 403s FIRST.
