...
#    Plus helper draft-agreement-pdfs v1 (no business logic). (v: 2026-08-17 — v754 / v40)
...
cd C:\vfo-react;          git tag -l 'live-*'         --sort=v:refname | Select-Object -Last 1   # (v: 2026-08-17 → live-152-planner-portal-sections)
cd C:\vfo-edge-functions; git tag -l 'backend-good-*' --sort=v:refname | Select-Object -Last 1   # (v: 2026-08-17 → backend-good-2026-08-17-v754)
...
**OPEN / OWED**
- **Test fixtures on Test Client 62 — re-derived, NOT carried:** plans **136 and 138 are GONE** (deleted during this session's testing) and the *"Download the ROI presentation"* bell is now **read**, so those parts of the prior OWED line are discharged. What actually exists now is **one** plan — **145** (program 4, `status='stopped'`, `deposit_refund_status='succeeded'`) + its 4 `client_tax_progress` rows + **one** `client_tax_specialists` row (Charles Riggs, `expert_id` 3553, `strategy` NULL) — plus **2 unread action-required bells** (*"Send the detailed tax plan meeting confirmation email…"* → Tray, *"Complete Client decision 2…"* → the test planner) and 4 unread dismissible FYIs. The **six** generated decks in the Drive *"ROI Presentations"* folder named `ROI Presentation - Test Client - 2026-08-17` are still there and must be removed **by hand**; the DB rows are a scripted delete. *(v: 2026-08-17 — every figure re-queried this session)*
...
- **Three UNTESTED items from the 2026-08-17 ROI-skip branch, still unexercised** — they were carried on a handoff block, and reached `main` with no landing place until #238 gave rule 5 one: (1) the **`retainer_first` skip route was click-tested BEFORE** the later `save-task` / `invoice-receipt` / `revshare-sweep` / `confirmation-email` edits landed, so that route has no post-change run; (2) the **decline path on a `meeting_first` plan** was never walked; (3) the **positional reorder was never viewed in Client Overview** — note it no longer lives in `overview-tax.ts`, having moved VERBATIM into `utils/tax-plan-steps.ts` on 2026-08-17; the code is unchanged, only its address is. The CHANGELOG's "click-tested end to end, 9/9 bells" covers `meeting_first` only. *(v: 2026-08-17)*
...
- **`vfo-admin-api`** — `index.ts` orchestrator + ~470 modular handler files in `actions/<group>/*.ts` + `router/dispatch.ts` + `router/webhooks.ts` + `middleware/auth.ts` + `utils/` + `constants/` + `integrations/`. Serves all 463 actions. *(v: 2026-08-17)*
