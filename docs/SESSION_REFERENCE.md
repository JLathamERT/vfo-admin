...
- **Two UNTESTED items from the 2026-08-17 ROI-skip branch:** (1) the **decline path on a `meeting_first` plan** was never walked; (2) `overview-tax.ts`'s **positional reorder has never been viewed in Client Overview**. **BOTH skip routes themselves were click-tested and pass** — `retainer_first` and `meeting_first`, both against v749, which already contained every Phase 2 backend change. *(v: 2026-08-17)*
