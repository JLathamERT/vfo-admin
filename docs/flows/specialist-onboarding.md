...
| `automation_SPECIALIST_ddcapprove` | AUTH (admin) | Stage 3 "DD Checklist Approved" | Drafts `SPECIALIST_ddc_approved` (**TBD body**); `ddc_review_status='approved'`; appends `ddc_review_log` `{approved}` |
...
The Stage 3 admin tracker (restructured 2026-06-04, `SpecialistOnboarding.jsx`) has a payment lead-in, then **Background Check**, **Due Diligence Checklist**, and **Final Revenue Share Proposal** sections, plus a **"Stage 3 progress"** rollup (green/grey/red bullets) above the final "Step 3 complete, moving to Step 4" auto-step. **Gate:** Stage 3 → Stage 4 when background-check **Passed** AND DD checklist **Approved** (rev-share finalization is independent and does not gate).
...
- Specialist fills the **public DDC form** `/specialist-ddc?token=<ddc_token>` (`SpecialistDdcPage.jsx`): text answers (bio, case studies, disclosures, audit support, exit strategies, sample-material notes) + file uploads (headshot, licenses, E&O, PLR/TOL, sample materials) to the **private `specialist-dd-materials`** bucket via signed upload URLs (gotcha #69). **Save progress + resume** (token stays valid), then **Submit for review**.
- **Request Help** → `/specialist-ddc-help` → non-dismissible Tracy notif → admin "Help received" bullet.
- Admin reviews via the **"View progress"** expander (all text answers + signed-URL file downloads via `ddcdownload`), then **Approved** (→ `SPECIALIST_ddc_approved`, **TBD body**, gotcha #72) or **Denied** (reason → `SPECIALIST_ddc_edits` email w/ resubmit link, clears `ddc_submitted_at`). Loops until approved; every round is a **paper-trail** bullet (`ddc_review_log`, gotcha #71).

**Final Revenue Share Proposal section:** an automated step records the specialist's email response (Happy / Further Questions). Happy → done. Further Questions → Tracy **Confirm** (locks as-is) or **Edit** (edited text → `rev_share_final_text`, original `rev_share_prepared` preserved — gotcha #70). "View final proposal" only appears once finalized; Stage 2's "View proposal" shows Original + Edited.
...
- `email_templates` (pipeline `SPECIALIST_ONBOARDING`, all editable incl. CC/BCC): `SPECIALIST_yes`, `SPECIALIST_no`, `SPECIALIST_step2_progress`, `SPECIALIST_denied`, `SPECIALIST_step3`, `SPECIALIST_step3_proceed`, `SPECIALIST_bg_receipt` (id 80, now **written** — DD checklist + Exhibit A), `SPECIALIST_bg_confirmation|card`, `SPECIALIST_bg_confirmation|ach`, `SPECIALIST_ddc_edits` (id 88, Denied reason + resubmit link), `SPECIALIST_ddc_approved` (id 87, **body still TBD** — gotcha #72), and the 4 reminder templates `SPECIALIST_sif_reminder`/`_revshare_reminder`/`_vote_reminder`/`_choice_reminder`. All 10 specialist emails end with the AI-PC / Proactive Coordinator footer (`VFO_SIGNATURE`).
...
- **Stages 4–5 automation** (contract/BoldSign, going-live) — still manual; no auto-promotion to the `experts` roster. (Stage 3's DD checklist is now an automated review loop; background-check Passed/Failed is still a manual admin click.)
- **`SPECIALIST_ddc_approved` email body is a TBD placeholder** (id 87) — fires on "DD Checklist Approved"; fill next chat (gotcha #72). (`SPECIALIST_bg_receipt` id 80 is now written.)
- **Stage 3 gate question:** Stage 3 currently advances on background-check **Passed** + DD **Approved** only; rev-share finalization is independent (can still show "Waiting" when Stage 3 completes) — confirm whether it should gate.
