...
| `specialist_vault_admin_list` / `_admin_download` | AUTH (admin) | Admin Search Specialists → Vault tab | Admin-only, by `expert_id` |
...
- Clears the "Create the VFO Specialist" Tracy notif + raises **"Add headshot (white background)"** + **"Add specialist bios"**.
...
**Admin vault view** — Search Specialists → click a specialist → tabs **Edit Specialist | Vault**. The Vault tab uses admin-only handlers `specialist_vault_admin_list` / `_admin_download` (by `expert_id`). A `?expert=<id>` deep-link auto-opens a specialist.

**Stage-5 completion gate + Tracy ladder.** The Create button does **NOT** complete the onboarding — completion (`status='completed'`) requires **BOTH** stage-5 checkboxes `headshot_added` + `bios_added` (gated in `save-progress.ts`). Tracy (`tnmiller@elitert.com`) action-required ladder (all non-dismissible):
- first invoice → **"Send VFO Skool invite"** + **"Create the VFO Specialist & send login"**;
- `skoolinvite` clears the first;
- `createspecialist` clears the second + raises **"Add headshot (white background)"** + **"Add specialist bios"**;
- each checkbox clears its matching notif.

Stage-5 progress task_keys: `skool_invite`, `intro_post`, `team_members_added`, `added_to_showroom`, `headshot_added`, `bios_added` (freeform keys in `specialist_onboarding_progress`).
