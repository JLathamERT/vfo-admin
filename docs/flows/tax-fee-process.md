...
- **The client's Proceed click at Client decision 2** (`implement-final-decision.ts`) records the decision and returns early with `no_fee_due: true`: **no acknowledgement email is drafted** (its copy would quote *"$0.00"* as the payment being processed) and **no charge chain runs**. The bell still fires — `TAX_impl_client_proceed`, message *"No implementation fee is due — the fee was amended so the retainer already paid covers the whole engagement. Nothing was charged."*
...
- **No corrected invoice is ever issued.** A fresh invoice only issues at the next payment, and there is none. Same reasoning as the decline case above.
...
- **`utils/tax-invoice-html.ts` and `send-agreement.ts` would print a `$0.00` implementation row** if either were re-run after such an amendment. Both are unreachable in the ordinary flow (the agreement is signed long before Tax 5, and no invoice issues without a payment), so neither was given a zero branch.
- **Decline at Client decision 2 on such a plan charges and refunds nothing** — VFO holds the whole retainer. Any partial refund is a hand Stripe refund, by decision (Jake, 2026-09-09).

- **A Refund click at Client decision 1 is unaffected** — it refunds the FULL retainer, as on any other plan.
...
**Live status** *(v: 2026-09-09)*. Backend shipped as `vfo-admin-api` **`v819`**; the frontend half was NOT yet published at the time of writing. **Proved end to end on sandbox fixture plan 186** (client 62, `$9,000` / `$4,500` retainer / 2-payment, wiped after): the `$4,499` refusal, the amend to `$4,500` → implementation `0` with shares `3,000` → `1,500 ×3` and an audit row, Client decision 1 clicked green, Tax 5b *"Keep"*, then the client's *"Yes - Proceed"* writing the decision and **nothing else** — no PaymentIntent, no checkout token, no receipt, **no `"$0.00"` draft in Gmail** — with Client Overview advancing to Tax 6 and the tab showing the single done row. **Petrus's plan 71 has since been amended for real through the portal** (total `29,000` → `14,500`, implementation `14,500` → `0`, audit row 18); his implementation decision email has not been sent, so **the first genuine `$0` Proceed has not happened yet**, and **Lana's amendment — the first real Tax 4 amend-to-`$0`, and the first retainer revenue share on re-scaled shares — waits on her ACH settling.** The `Confirmed` arm is UI-unreachable (the tab only ever sends `'Undecided'`) and a `$0` Decline has never been clicked.
