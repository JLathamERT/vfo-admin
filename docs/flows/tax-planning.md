...
7. Flips `ready_for_tax3_email_sent='Yes'` **and, on `confirm_date`, PERSISTS `tax3_meeting_date` / `tax3_meeting_time` / `tax3_meeting_timezone`.** Those three columns are the only source for the step's **"Meeting booked for M/D/YYYY at h:mm AM TZ"** pill, for the nightly sweep's assess-reminder window and for that email's `[Meeting Date]`/`[Meeting Time]`/`[Meeting Timezone]` tokens. The decline path never writes them. **Legacy rows survive display-only:** plans confirmed before v717 have no trio, so the pill falls back to the stored progress status and the old `'Yes - Confirmation email (date TBC)'` value is deliberately retained in the FE colour map.
   **RESCHEDULE (NEW 2026-08-16, v747).** The booked pill now carries a **Reschedule** button that reopens the same date/time/timezone form pre-filled from the stored trio and re-sends the SAME `TAX_readyfortax3|Yes` template. **When the date actually changes and `tax3_assess_reminder_sent_at` was already stamped, the handler NULLs it** so the planner's before-meeting assess reminder re-arms against the new date — that ladder counts *backwards from the meeting* (#359), so unlike every other reschedule in this family it SHOULD re-arm (#404). The sweep's own guards (form submitted / vault drop / date in range) still apply, so a plan whose assess form has since been submitted gets nothing.
...
## Step 1¼ — Generate and download presentation (Tax 2 - Deeper Dive, added 2026-08-06 v706; TWO-YEAR v2 rewrite 2026-08-07 v713; **FOUR-TOTAL INPUTS + ALWAYS-SINGLE-YEAR 27-slide deck as of 2026-08-17**)
...
**What Generate does, end to end.** Reads the plan + `assess_form` + the **plan-level** risk grade (`client_tax_progress` where `tax_specialist_id IS NULL`, task "Client risk profile complete", parsed `/Risk\s*([1-5])/`) → downloads **`ROI-template-master-v3.pptx`** from the private **`presentation-templates`** bucket → substitutes **21 `{{TOKENS}}`** across the parts **1/20/23/24/25/27/28/29** (XML-escaped) → **drops BOTH year-appendix slides from the package, unconditionally (see below)** → repositions the slide-20 `upArrow` to the client's risk band off a hardcoded EMU table (1→801612, 2→2566499, 3→4349395, 4→6126289, 5→7900180) → re-zips with **fflate** (untouched entries **stored at level 0**, only the 8 edited slide XMLs deflated — the ~27 MB of PNGs is never recompressed) → uploads to Google Drive **multipart with metadata `mimeType='application/vnd.google-apps.presentation'`**, which is what makes Drive convert it into a **real Google Slides file**, into an **"ROI Presentations"** folder under `GOOGLE_DRIVE_FOLDER_ID` → sets sharing **role=writer type=anyone** (deliberate, user-approved over view-only after a security review) → reads `webViewLink` → stamps `client_tax_plans.generated_presentation_{drive_id,url,at,by}` → returns `{success, url, generated_at}`. Runs ~30–60s, which is why `src/lib/api.js` grants this action **90s** via `LONG_TIMEOUT_ACTIONS` (writes still never retry, #30).

**THE DECK IS ALWAYS SINGLE-YEAR AND ALWAYS 27 SLIDES as of 2026-08-17 (#374).** The two-year story below is retained because the template still contains its slides and older generated decks still exist in Drive.

- **Slide 24 is the whole numeric story now, and it carries no year.** The `Your investment: {{INVEST_YEARS}}` header (added v3 / v729) was **deleted from the master on 2026-08-17** and the twelve left-column shapes below it shifted up 0.59in to close the gap; the right-hand Net Tax Savings panel sits higher and did not move. The year had already been **removed** from the "Estimated Tax Savings" / "Net Tax Savings" / "Plan Investment" labels. Do not put the year back. **`INVEST_YEARS` is now unconditionally `String(currentYear)` AND has no site left in the master** — the old `<year> + <year2>` alternative is gone and so is the box it lived in, so the token is computed, substituted against nothing, and deliberately dead. `YEAR`/`YEAR2` are still substituted but appear only on the two appendix parts, which no generated deck contains. **Net effect: a generated deck contains no year anywhere.**
- **BOTH appendix slides are dropped from EVERY package** — `ppt/slides/slide28.xml` (year 1) and `ppt/slides/slide29.xml` (year 2), not blanked, removed. Per slide, four things go together and missing any one makes PowerPoint call the file corrupt: the `<p:sldId>` in `ppt/presentation.xml`, the `<Relationship>` in `ppt/_rels/presentation.xml.rels`, the `<Override>` in `[Content_Types].xml`, and the part plus its `.rels` in the zip.
...
- **`SLIDE_PARTS` still lists 28 and 29** so their tokens are substituted before the parts are dropped — that is what keeps the leftover-token guard uniform across all 8 parts. Do not remove them from the list.
- **⚠ FILENAMES ARE NOT POSITIONS.** `slide28.xml` / `slide29.xml` **DISPLAY at 25 and 26** — `sldIdLst` order is `1–24, 28, 29, 25, 26, 27`, pushing "Choose Who You Pay" / Double-Guarantee / Congratulations out to display **27/28/29**. Parts are addressed by filename; never "correct" `SLIDE_PARTS` to display numbers.
- **Consequence for anyone reading a generated deck:** it is **27 slides, always**, and no consumer may assume 29 slides or fixed display positions past display 24. This is the generated OUTPUT only — the template in the bucket still has all 29.

**Values on the deck (rewritten 2026-08-17 — the derivation is INVERTED at the top).** Everything comes from the assess form's four entered totals: **`W`=`taxes_without_plan`, `T`=`taxes_with_plan`, `C`=`cash_outlay`, `F`=`fee`**. `taxes_with = T` is now **ENTERED** and `gross = W − T` is **DERIVED** — the exact inverse of the old model, where the gross was summed from strategies and `taxes_with` fell out of it. Then `invest = C`, `net = gross − invest − F`, `outlay_plus_fee = invest + F`, `fee_half = F/2`, `pct = round(gross/W×100)`, `year` = the **generation-time** calendar year, `year2 = year+1`. The six appendix tokens still substitute but no longer carry per-year data: `GROSS_Y1`/`INVEST_Y1`/`NET_Y1` are the whole-plan figures and `GROSS_Y2`/`INVEST_Y2`/`NET_Y2` are `$0` — kept only so all 21 tokens resolve before their slides are dropped. Money is whole-dollar with commas — **`FEE_HALF` is the only cents-tolerant token**; negatives use accounting parentheses, and `NET_BENEFIT` can legitimately come out negative.
...
> **RESCHEDULE (2026-08-16, v747).** The sent pill carries a **Reschedule** button that reopens the same date/time/timezone form pre-filled and re-sends the same `TAX_highlevelmeeting_confirm|Yes` template. **Admins only since 2026-08-18** — because it re-sends, it sits inside the planner lock, so the planner portal shows the locked row instead of the pill-plus-button. **Frontend-only — no handler change was needed**, because step 4 below already nulls `tax4_meeting_reminder_last_sent_at` on every confirm, so the planner's `TAX_planner_tax4_steps_needed` bell re-arms against the new date by design. This is the exception in the #404 family: here re-arming is already the correct behaviour.
...
| **13** | Tax 1 | [vault/upload-notify.ts](C:/vfo-edge-functions/supabase/functions/vfo-admin-api/actions/vault/upload-notify.ts) **and, as of 2026-08-16, [tax/returns-already-have.ts](C:/vfo-edge-functions/supabase/functions/vfo-admin-api/actions/tax/returns-already-have.ts)** | The client's **tax returns** land (portal upload OR `/tax-upload` link), **or an admin clicks "Already have tax returns"**, and the stamped plan has **no** planner (rule `TAX_returns_allocate_team_member`, `dedupe:"unread"`). Same title, **message variant only** ("…are already on file…") | **Tracy + Tray** | **false** | Action-required ("Allocate a team member for \<client\>") | `allocate-planner.ts` when an allocation is **SET** (`clearAllocateTeamMemberBells`) — a clear-to-nobody deliberately leaves it standing |
...
## "Assess tax planning opportunities (and enter presentation details)" form-step (added 2026-07-22; structured strategy form 2026-07-29 v677; **FOUR FLAT TOTALS as of 2026-08-17**)
...
- **UI (`AssessTaxForm` in [TaxPrioritiesTab.jsx](src/components/admin/tax/TaxPrioritiesTab.jsx)) — FOUR MONEY INPUTS as of 2026-08-17.** A single **"Enter Details"** button → the inline form, which is now four required dollar inputs in a 2-column grid and nothing else: **"Total Tax without the Plan"** (`taxes_without_plan`), **"Total Tax with the Plan"** (`taxes_with_plan`), **"Total Cash Outlay for the Strategies"** (`cash_outlay`), **"VFO Services Tax Planning Fee"** (`fee`). Below them sits a live muted summary line — *"Gross Tax Benefit: $X · Net Benefit: $Y · Tax Reduced: Z%"* — computed as `gross = without − with`, `net = gross − outlay − fee`, `pct = round(gross/without×100)`, rendering **"—"** until both tax totals are valid numbers. **Nothing derived is stored**; the deck recomputes it all (gotcha #306). Submit calls `tax_save_assess_form`, then `saveTask 'Completed'` greens the step. Submitted state = a green "Submitted" pill + date + a chevron-expand read-only view of the same four values plus the summary line. **THE READ-ONLY VIEW HAS THREE BRANCHES, one per historical shape:** the current four totals; the 2026-07-29→2026-08-17 **strategy list** (fee + total-taxes boxes + the old Investment Costs / Gross Tax Plan Savings / Net Tax Savings summary table, rendered exactly as it always was — `assessComputeRows` / `assessSectionSum` / `AssessSummaryTable` survive **only** for this); and the original `{question_1}` single-question view. Shape detection is `taxes_with_plan != null` → new, else `Array.isArray(strategies)` → old structured, else legacy. Member readOnly = the pill only.
...
- **Handler:** [`tax_save_assess_form`](../../supabase/functions/vfo-admin-api/actions/tax/save-assess-form.ts) — AUTH, in `ADMIN_ONLY_ACTIONS` + `TAX_PLANNER_ALLOWED_ACTIONS`, `denyIfNotPlannerPlan` group-scope guard. Validates + normalizes **all four totals, each required, non-negative and rounded to 2 dp**, then the cross-field rule that **`taxes_with_plan` must be STRICTLY less than `taxes_without_plan`** (which is also what guarantees `taxes_without_plan > 0` for the deck's division). **The five 400 messages are mirrored VERBATIM in the FE `AssessTaxForm.handleSubmit`, in this order** (#306): *"VFO Services Tax Planning Fee is required"*, *"Total Tax without the Plan is required"*, *"Total Tax with the Plan is required"*, *"Total Cash Outlay for the Strategies is required"*, *"Total Tax with the Plan must be less than Total Tax without the Plan"*. Stores ONLY `{ fee, taxes_without_plan, taxes_with_plan, cash_outlay }` into `client_tax_plans.assess_form` (jsonb) — **the `strategies` array is gone from new submissions and the old per-strategy messages no longer exist on either side** — + stamps `assess_form_submitted_at` + `assess_form_submitted_by` (session email). The handler **rebuilds the whole jsonb**, so a re-save is the only upgrade path for a legacy row; there is no backfill. **NO chains / emails.** Migration `20260722120000_client_tax_plans_assess_form.sql`; `tax_load_plans` uses `select("*")` so no loader change.
