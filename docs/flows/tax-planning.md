...
**Every step names an owner.** `Step.owner` is a display string: the **allocated planner by name** where one exists (passed in as `ctx.plannerName`, batch-resolved from `tax_planners`), otherwise a role — `Tray` (returns request, green/red light, HLM confirm, ready-for-Tax-3), `Tracy / Tray` (the `tax_planner_select` allocation), `Admin` (deposit, risk profile), `Client`, `Anton (CEO)` (countersign), and `System` for the automated cascade steps nobody should be chased about. **PF-owned steps are the exception: they emit the `PF_OWNER` token `"__PF__"`, never a name**, because the engines run before `enrichClients` and cannot see `clients.assigned_pf` — `actions/clients/overview.ts` resolves the token afterwards, and it is the only place that may. Gotcha **#415**.
