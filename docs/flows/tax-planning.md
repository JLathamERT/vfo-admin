...
- **Slide 24 is the whole numeric story now.** Header `Your investment: {{INVEST_YEARS}}` **as of v3 / v729** (box widened to 4.70in, `wrap="none"`, so it can never wrap), and the year was **removed** from the "Estimated Tax Savings" / "Net Tax Savings" / "Plan Investment" labels. Do not put the year back. **`INVEST_YEARS` is now unconditionally `String(currentYear)`** — the old `<year> + <year2>` alternative is gone. `YEAR`/`YEAR2` are still substituted but appear only on the two appendix parts, which no generated deck contains.
