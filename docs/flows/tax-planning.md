...
- **Admin mode** computes this in the FE: `taxSpecialistOptions` / `taxShortBio` in `TaxPrioritiesTab.jsx` read the cached `load_data` payload's `experts` + `ecosystems` (`ClientDetail` now passes an `ecosystems` prop alongside `specialists`). If the assignment rows are missing it falls back to the unfiltered roster rather than an empty picker.
