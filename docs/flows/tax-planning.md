...
```
ROI Mtg booked   →  Tax 3 Decision  →  /tax-decide      →  PCADMIN pricing  →  BoldSign agreement  →  Stripe customer  →  /tax-pay  →  Stripe payment  →  Confirmation + Invoice/Receipt  →  Tax 4 Continue/Stop  →  Implementation auto-charge
...
| 7 | **117 Client risk profile complete** | 7 | **84 Allocate Team Member / Tax Planner** (`tax_planner_select`) |
| 8 | **118 Allocate Team Member / Tax Planner** (`tax_planner_select`) | | |

> **Tasks 84/118 were RENAMED on 2026-08-10** — "Allocate to Advanced Tax Planner" → **"Allocate Team Member / Tax Planner"** (migration `20260810130100_tax_roi_step_renames.sql`, DML only, no id or order change). Team Members are now allocatable and are the deliberate FIRST holder of a plan — see [the ROI meeting hand-off chain](#the-roi-meeting-hand-off-chain-2026-08-10-v717) below and gotcha #294 as amended. The FE matches this step by **sentinel (`tax_planner_select`) first** with the old and new names both kept as name fallbacks, which is what made the rename safe to apply before the frontend deploy.
...
**TWO buttons, not three.** **Send email (with date)** (green) opens the date / time / timezone form, with Send **disabled until a date is entered** and a Cancel beside it; **No – Declined email** (red) is unchanged and opens the inline red-dashed `[DECLINE_REASON]` textarea. **"Send email – date not confirmed" is GONE** — `decision:"confirm_no_date"` now returns **400 `"A meeting date is required"`**, deliberately kept as its own named branch so an old cached bundle gets a legible refusal rather than a generic one.
...
8. **Hand-off chain, fail-soft (gotcha #357).** Either decision clears this client's unread action-required bell titled `Book the ROI meeting for%`; then, on `confirm_date` with a **Team Member** allocated, it fires `TAX_team_member_allocate_planner` — "open the client and allocate the Tax Planner who will run the plan". Wrapped in one try/catch: a bell can never fail an email that has already been drafted.
...
**What Generate does, end to end.** Reads the plan + `assess_form` + the **plan-level** risk grade (`client_tax_progress` where `tax_specialist_id IS NULL`, task "Client risk profile complete", parsed `/Risk\s*([1-5])/`) → downloads **`ROI-template-master-v2.pptx`** from the private **`presentation-templates`** bucket → substitutes **20 `{{TOKENS}}`** across the parts **1/20/23/24/25/27/28/29** (XML-escaped) → repositions the slide-20 `upArrow` to the client's risk band off a hardcoded EMU table (1→801612, 2→2566499, 3→4349395, 4→6126289, 5→7900180) → re-zips with **fflate** (untouched entries **stored at level 0**, only the 8 edited slide XMLs deflated — the ~27 MB of PNGs is never recompressed) → uploads to Google Drive **multipart with metadata `mimeType='application/vnd.google-apps.presentation'`**, which is what makes Drive convert it into a **real Google Slides file**, into an **"ROI Presentations"** folder under `GOOGLE_DRIVE_FOLDER_ID` → sets sharing **role=writer type=anyone** (deliberate, user-approved over view-only after a security review) → reads `webViewLink` → stamps `client_tax_plans.generated_presentation_{drive_id,url,at,by}` → returns `{success, url, generated_at}`. Runs ~30–60s, which is why `src/lib/api.js` grants this action **90s** via `LONG_TIMEOUT_ACTIONS` (writes still never retry, #30).

**The deck is TWO-YEAR as of 2026-08-07 (v713) — one combined slide plus two single-year slides.**

- **Slide 24 is the COMBINED view and now says so.** Header `Your investment – {{YEAR}} + {{YEAR2}}` (box widened to 4.70in, `wrap="none"`, so it can never wrap), and the year was **removed** from the "Estimated Tax Savings" / "Net Tax Savings" / "Plan Investment" labels — the values there were always two-year totals and the labels used to contradict them. Do not put the year back.
...
| **13** | Tax 1/2 | [allocate-planner.ts](C:/vfo-edge-functions/supabase/functions/vfo-admin-api/actions/tax/allocate-planner.ts) | A **Team Member** is allocated and the ROI email has NOT gone out (rule `TAX_book_roi_meeting`) | **Tray** (`tvaldes@elitert.com`) | **false** | Action-required ("Book the ROI meeting for \<client\>") | `ready-for-tax3.ts` on **either** decision — `.ilike("title","Book the ROI meeting for%")` |
| **14** | Tax 1/2 | [allocate-planner.ts](C:/vfo-edge-functions/supabase/functions/vfo-admin-api/actions/tax/allocate-planner.ts) (Team Member allocated **after** the ROI email) **and** [ready-for-tax3.ts](C:/vfo-edge-functions/supabase/functions/vfo-admin-api/actions/tax/ready-for-tax3.ts) (`confirm_date` while a Team Member holds the plan) | rule `TAX_team_member_allocate_planner` | **the allocated Team Member** (via `notifyAllocatedPlanner`, planner-portal link) | **false** | Action-required ("Allocate a Tax Planner for \<client\>") | `allocate-planner.ts` on **any** allocation change |
| **15** | Tax 1/2 | [allocate-planner.ts](C:/vfo-edge-functions/supabase/functions/vfo-admin-api/actions/tax/allocate-planner.ts) | A **Tax Planner** is allocated while `assess_form_submitted_at` is NULL (rule `TAX_planner_assess_needed`) | **the allocated planner** | **false** | Action-required ("Enter the tax planning opportunities for \<client\>") | `allocate-planner.ts` (any allocation change) **and** `save-assess-form.ts` (on submit) |
| **16** | Tax 2 | [save-assess-form.ts](C:/vfo-edge-functions/supabase/functions/vfo-admin-api/actions/tax/save-assess-form.ts) | **FIRST** submission of the assess form only (a later edit is silent, #314) — rule `TAX_assess_completed_pf` | **assigned PF**, resolved through `taxPfLoginEmail` (the PORTAL LOGIN address, not the `pf-emails.ts` correspondence map) | true | FYI ("Tax planning opportunities entered for \<client\>") | click / Done |
...
| [allocate-planner.ts](C:/vfo-edge-functions/supabase/functions/vfo-admin-api/actions/tax/allocate-planner.ts) | unread TAX for `client_id`, `dismissible=false`, titles `Allocate a Tax Planner for%` **and** `Enter the tax planning opportunities for%` — on **ANY** allocation change, set or clear, and **BEFORE** the new bell is fired | #14 / #15 |
| [ready-for-tax3.ts](C:/vfo-edge-functions/supabase/functions/vfo-admin-api/actions/tax/ready-for-tax3.ts) | unread TAX for `client_id`, `dismissible=false`, title `Book the ROI meeting for%` — on **either** decision | #13 |
| [save-assess-form.ts](C:/vfo-edge-functions/supabase/functions/vfo-admin-api/actions/tax/save-assess-form.ts) | unread TAX for `client_id`, `dismissible=false`, title `Enter the tax planning opportunities for%` | #15 |

> **Rows 13–15 are the ROI hand-off chain and their TITLES are load-bearing (gotcha #357).** They are `dismissible:false`, which per #179 means `mark_notification_read` refuses them — the ONLY thing that ever clears one is the matching prefix write above. Reword a title without moving its clear, or raise one of them from a new handler without shipping the clear, and the bell becomes permanent and invisible to everyone who did not receive it.

### Email-only "nag" ladders (no persistent in-app piece)

All live in [revshare-sweep.ts](C:/vfo-edge-functions/supabase/functions/vfo-admin-api/actions/tax/revshare-sweep.ts) (cron `tax-revshare-sweep-daily`, 02:30 UTC). 48h tier = a sandbox-aware **Gmail draft to the client** (with the decision buttons re-rendered); 96h tier = the shared-admin FYI in the table above (#10/#11/#12). **There is no in-app notification at the 48h tier, and the 96h tier is a click-to-dismiss FYI, not persistent.** **The last row of the table is the exception on both counts (2026-08-10):** it is addressed to the **allocated planner** rather than the client, it has **no 96h tier at all**, and its delay is counted **backwards from the booked meeting date** rather than forwards from a trigger (#359).
...
- **NOTIFICATIONS as of 2026-08-10 (v717) — it is the last link in the ROI hand-off chain (gotcha #357).** The handler now reads the plan **BEFORE** the update (`id, client_id, program_id, assess_form_submitted_at`) because the prior stamp is the only thing separating a first submission from an edit. After a successful save it (a) clears this client's unread action-required bells titled **`Enter the tax planning opportunities for%`** — the one `allocate-planner.ts` raised when the planner was allocated — and (b) **on the FIRST submission only** fires the FYI **`TAX_assess_completed_pf`** ("Tax planning opportunities entered for \<client\>") to the assigned PF. **The PF address here is `taxPfLoginEmail` (`utils/tax-notify.ts`) — the PORTAL LOGIN address — not the `utils/pf-emails.ts` correspondence map the email templates use**; a bell routed to a non-login address is unreadable. An edit-resave is deliberately silent (#314). The whole block is one try/catch — a bell can never lose a submitted form.

## The ROI meeting hand-off chain (2026-08-10, v717)

**The problem it solves.** A plan used to be allocated straight to the Advanced Tax Planner who would run it — which meant the ROI meeting nobody had booked yet was somebody's job sitting in nobody's queue. The tax track now models the hand-off explicitly, and the mechanism is a chain of **action-required bells** raised and cleared across four handlers. Because `dismissible:false` rows are refused by `mark_notification_read` (#179), the ONLY thing that ever clears one is a completing handler writing `read=true` directly, matched on **`pipeline='TAX'` + `client_id` + `dismissible=false` + `read=false` + `title ILIKE '<prefix>%'`** — which makes the three title prefixes a shared API (gotcha **#357**).

**The happy path, in order.**

1. **Allocate the coordinator.** An admin (or a planner, from their own portal) picks a **Team Member** on the "Allocate Team Member / Tax Planner" step. `tax_allocate_planner` writes `client_tax_plans.tax_planner_id` and raises **`Book the ROI meeting for <client>`** to **Tray** (`TAX_book_roi_meeting`, `tvaldes@elitert.com`), plus an FYI to the coordinator retitled *"You have been allocated as Team Member for \<client\>"*.
2. **Book the meeting and send the confirmation.** `automation_TAX_readyfortax3` on the (now first) **"ROI Meeting booked - Send Confirmation Email"** step drafts the client email, **persists `tax3_meeting_date`/`_time`/`_timezone`** (#358), **clears Tray's bell** (either decision — declining also retires it), and, because a Team Member still holds the plan, raises **`Allocate a Tax Planner for <client>`** to that coordinator (`TAX_team_member_allocate_planner`).
3. **Hand the plan on.** The coordinator re-opens the allocation step and replaces themselves with a real **Tax Planner**. `tax_allocate_planner` **clears the "Allocate a Tax Planner" bell first**, then — because `assess_form_submitted_at` is still null — raises **`Enter the tax planning opportunities for <client>`** to the new planner (`TAX_planner_assess_needed`).
4. **The planner does the work.** `tax_save_assess_form` clears that bell and FYIs the assigned PF (`TAX_assess_completed_pf`) on the **first** submission only.
5. **If they don't, the sweep chases them.** Two days before `tax3_meeting_date` (configurable — and **the delay counts DOWN**, #359), the nightly tax sweep drafts `TAX_tax3_assess_reminder` to the planner, Cc'ing their group's Team Members and the assigned PF.

**The branches that are easy to miss.**

- **`tax_allocate_planner` clears BEFORE it re-fires, and it clears on ANY genuine change — including a clear-to-nobody.** So a re-allocation never leaves the previous holder chasing a step that is no longer theirs.
- **Allocating a Team Member to a plan whose ROI email ALREADY went out** (`ready_for_tax3_email_sent='Yes'` AND `ready_for_tax3_decision='Yes'`) **skips Tray's leg entirely** and goes straight to "Allocate a Tax Planner".
- **Allocating a Tax Planner whose assess form is already submitted** falls through to the original `TAX_planner_allocated` FYI, byte-identically to pre-v717 behaviour.
- **`notifyAllocatedPlanner` gained an optional `dismissible`** (default `true`), so all eight pre-existing planner bells are unchanged; only the two chain bells pass `false`.
- **Every leg is fail-soft** — a bell failure can never fail an allocation, an already-drafted email or a saved form.
...
**Frontend surfaces.** The allocation picker no longer filters Team Members out; each option is suffixed `— Team Member` and a selected Team Member shows an amber chip. `PlannerClientsList.jsx`'s group filter labels carry the same suffix. `templateMeta.js` gained the `TAX_tax3_assess_reminder` entry (*"To: Allocated Tax Planner · Auto-Cc: group Team Members + PF"*).

**Deploy ordering matters here.** The two real coordinators' role flip (`20260810130200`, Katie Williams 14 + Bridger Silvester 15) is **deploy-time DML** — under the pre-deploy frontend bundle a Team Member is filtered out of every picker, so flipping the rows before `npm run deploy` would break allocation for both partnerships. Apply it immediately after.
...
### Ten planner notification bells (six NEW 2026-07-22, a seventh 2026-07-23, an eighth 2026-07-23 evening, a ninth and tenth 2026-08-10)

All resolve `tax_planners.email` from the plan's `tax_planner_id` via `notifyAllocatedPlanner` (`utils/tax-planner-notify.ts`; try/catch-safe, skips null client/planner/email; bell link `/tax-planner/client/<client_id>?program=<pid||1>`). Seeded in `notification_rules` (area 'Tax Planners', `default_recipients '["ALLOCATED_TAX_PLANNER"]'` placeholder — the real email is passed per-call) by migrations `20260722130000_tax_planner_step_notifications.sql` (six) + `20260723120000_tax_additional_info_request.sql` (the seventh) + `20260723210000_impl_charged_bells.sql` (the eighth) + `20260810130300_tax_roi_notification_rules.sql` (the ninth and tenth). **`notifyAllocatedPlanner` gained an optional `dismissible` on 2026-08-10** (`args.dismissible !== false`, so the default stays `true` and the first eight bells are byte-identical); the two ROI-chain bells pass `false`, which per #179 means they can only be cleared by the matching title-prefix write in their completing handler (#357). **The bell TITLES now name the client** — `notifyAllocatedPlanner` substitutes a `[Client Name]` token (lazy `clients` lookup by `plan.client_id`, only when the token is present) so a planner sees WHICH client, and because dedupe `"unread"` is title-keyed the per-client titles scope dedupe per client (gotcha #272):
...
| `TAX_team_member_allocate_planner` | `allocate-planner.ts` **and** `ready-for-tax3.ts` | a **Team Member** holds the plan and must hand it on — raised when the ROI confirmation email goes out with a date, or when a Team Member is allocated to a plan whose ROI email already went out. **NINTH planner bell, and the first that is `dismissible:false`** (action-required); cleared by `allocate-planner.ts` on any allocation change. Title `Allocate a Tax Planner for [Client Name]`. Gotcha #357 |
| `TAX_planner_assess_needed` | `allocate-planner.ts` | a **Tax Planner** is allocated while `assess_form_submitted_at` is NULL. **TENTH planner bell, also `dismissible:false`**; cleared by `allocate-planner.ts` (any allocation change) and by `save-assess-form.ts` on submit. Title `Enter the tax planning opportunities for [Client Name]`. Gotcha #357 |
