...
## Step 1¼ — Generate and download presentation (Tax 2 - Deeper Dive, added 2026-08-06 v706; TWO-YEAR v2 rewrite 2026-08-07 v713; four-total inputs + always-single-year 2026-08-17; **TWO-YEAR MODE RESTORED — 27 OR 29 slides, decided by the form, as of 2026-08-18 (v759, template v4)**)
...
**What Generate does, end to end.** Reads the plan + `assess_form` + the **plan-level** risk grade (`client_tax_progress` where `tax_specialist_id IS NULL`, task "Client risk profile complete", parsed `/Risk\s*([1-5])/`) → downloads **`ROI-template-master-v4.pptx`** from the private **`presentation-templates`** bucket → substitutes **21 `{{TOKENS}}`** across the parts **1/20/23/24/25/27/28/29** (XML-escaped) → **in single-year mode only, performs the slide-24 header surgery and drops BOTH year-appendix slides (see below); in two-year mode drops nothing** → repositions the slide-20 `upArrow` to the client's risk band off a hardcoded EMU table (1→801612, 2→2566499, 3→4349395, 4→6126289, 5→7900180) → re-zips with **fflate** (untouched entries **stored at level 0**, only the edited XMLs deflated — the ~27 MB of PNGs is never recompressed) → uploads to Google Drive **multipart with metadata `mimeType='application/vnd.google-apps.presentation'`**, which is what makes Drive convert it into a **real Google Slides file**, into an **"ROI Presentations"** folder under `GOOGLE_DRIVE_FOLDER_ID` → sets sharing **role=writer type=anyone** (deliberate, user-approved over view-only after a security review) → reads `webViewLink` → stamps `client_tax_plans.generated_presentation_{drive_id,url,at,by}` → returns `{success, url, generated_at}`. Runs ~30–60s, which is why `src/lib/api.js` grants this action **90s** via `LONG_TIMEOUT_ACTIONS` (writes still never retry, #30).

**THE DECK IS 27 OR 29 SLIDES, AND `assess_form.year2` IS THE ONLY THING THAT DECIDES (2026-08-18, #374).** Between 2026-08-17 and 2026-08-18 it was *always* 27; that is now the **single-year** branch of two, and the branch condition is the mere PRESENCE of the nested `year2` group — nothing else, no flag, no column.

- **`year2` ABSENT → single-year, 27 slides.** Both appendix slides are dropped AND slide 24's year header is deleted with the twelve shapes below it shifted up. Byte-for-byte the same output the always-single-year build produced.
- **`year2` PRESENT → two-year, 29 slides.** Nothing is dropped, both appendix slides ship, and slide 24 keeps its header reading **"Your investment: 2026 + 2027"**.
- **⚠ THE SINGLE-YEAR LAYOUT IS NOW PRODUCED AT GENERATION TIME, NOT BAKED INTO THE MASTER.** This is an INVERSION of the 2026-08-17 arrangement. Template **v4 carries the header**, and `dropInvestYearsHeader()` in the handler reproduces the old static layout on the fly: it deletes the one `<p:sp>` containing `{{INVEST_YEARS}}` and shifts every remaining `<p:sp>` whose `<a:off>` has **`x < 4800000`** (left column only — the right "Net Tax Savings" panel starts at `x=4876920` and must not move) and **`y >= 1773000`** up by **`HEADER_SHIFT_EMU = 539650`**. Exactly **1** deletion and exactly **12** shifts are asserted; either count being wrong is a loud 500, not a silently holed deck. It runs on the RAW xml **before** substitution, because the shape is located by the literal token substitution is about to erase. **These four constants are COUPLED TO TEMPLATE v4** — they were measured off v4's `slide24.xml` and byte-verified to reproduce the old live master's `slide24.xml`. A template rebuild MUST re-derive them; nothing else in the package pins them.
...
- **`SLIDE_PARTS` lists 28 and 29 in BOTH modes** so their tokens are substituted whether or not the parts survive — that is what keeps the leftover-token guard uniform across all 8 parts. Do not remove them from the list, and do not make the list conditional.
...
- **Consequence for anyone reading a generated deck:** the slide count is **not fixed** — read `year2` on the plan before assuming either shape, and never assume fixed display positions past display 24. This is the generated OUTPUT only — the template in the bucket always has all 29.
...
- **FEE ATTRIBUTION — the whole fee lands on YEAR 1 and none on year 2:** `NET_Y1 = GROSS_Y1 − INVEST_Y1 − F`, `NET_Y2 = GROSS_Y2 − INVEST_Y2`. That is the deck's original rule and the reason **slide 29's One-Time Fee is a literal `$0` in the template, not a token** — there is no `PLANNING_FEE_Y2`, and `make_v4.py` asserts one has not crept back in.
- **Every combined figure is the SUM of the per-year figures**, computed from them specifically so the two can never drift: `TAXES_WITHOUT = W1+W2`, `TAXES_WITH = T1+T2`, `GROSS_BENEFIT = GROSS_Y1+GROSS_Y2`, `CASH_OUTLAY = INVEST_Y1+INVEST_Y2`, `NET_BENEFIT = NET_Y1+NET_Y2`, `OUTLAY_PLUS_FEE = CASH_OUTLAY + F`, `FEE_HALF = F/2`, `PCT_REDUCED = round(GROSS_BENEFIT/(W1+W2)×100)`.
...
- In single-year mode the six appendix tokens still substitute — `GROSS_Y1`/`INVEST_Y1`/`NET_Y1` carry the whole-plan figures and the `_Y2` trio are `$0` — kept only so all 21 tokens resolve before their slides are dropped.
- Money is whole-dollar with commas — **`FEE_HALF` is the only cents-tolerant token**; negatives use accounting parentheses, and `NET_BENEFIT` (and `NET_Y1`, which carries the whole fee) can legitimately come out negative.
