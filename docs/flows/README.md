...
| [tax-fee-process.md](tax-fee-process.md) | The revised tax fee/pricing flow (2026-08-25, `client_tax_plans.fee_process_version='2026-08-25'`): one engagement total goes in and the retainer, the initial/final retainer split, the implementation fee and every revenue-share leg are derived server-side in whole cents. Above $30,000 the plan becomes a **3-payment** shape ($15,000 initial retainer → final retainer collected off-session on the client's Client decision 1 click → implementation). Also covers the Tax 4 "Amend fee" / Tax 5b "Amend implementation fee" re-pricing steps and the conversion back to a 2-payment plan. Legacy plans (`fee_process_version IS NULL`) are untouched. |
