...
export default function TaxPayPage() {
  ...
  async function loadPaymentData(token) {
    try {
      const res = await fetch(API_URL, {
        ...
        body: JSON.stringify({ action: 'automation_TAX_loadpayment', token }),
      })
  ...
  async function handleChoice(method) {
    ...
    try {
      const res = await fetch(API_URL, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'automation_TAX_stripecheckout', token: searchParams.get('token'), method }),
      })
      const d = await res.json()
  ...
  return (
    <TokenShell>
      <div style={pageContainerStyle}>
        ...
        <h1 style={{ ...titleStyle, fontSize: '22px', textAlign: 'center', marginBottom: '8px' }}>VFO Tax Planning Payment</h1>
        ...
        <p style={{ ...subtitleStyle, textAlign: 'center', marginBottom: '32px', fontSize: '13px', color: 'var(--vfo-muted)' }}>
          {data.payment_label || 'Tax Planning Retainer'} · {data.client_name}
        </p>
        ...
        <OptionCard
          ...
          amount={baseAmount}
          breakdown={[
            { label: 'Tax Planning Retainer', value: `$${formatMoney(baseAmount)}`, valueColor: 'var(--vfo-ink-2)' },
