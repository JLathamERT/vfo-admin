...
export default function AdvisorPayPage() {
  ...
  async function loadPaymentData(token) {
    try {
      const res = await fetch(API_URL, {
        ...
        body: JSON.stringify({ action: 'automation_ADVISOR_loadpayment', token }),
      })
  ...
  async function handleChoice(method) {
    ...
    try {
      const res = await fetch(API_URL, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'automation_ADVISOR_stripecheckout', token: searchParams.get('token'), method }),
      })
      const d = await res.json()
    ...
    <TokenShell>
      <div style={pageContainerStyle}>
        ...
        <OptionCard
          ...
          amount={baseAmount}
          breakdown={[
            { label: 'Advisor Onboarding', value: `$${formatMoney(baseAmount)}`, valueColor: '#243757' },
