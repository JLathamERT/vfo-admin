...
export default function ClientLogin() {
  ...
  async function handleLogin(e) {
    ...
    try {
      ...
      const data = await callApi('client_login', { email: emailVal, passcode: passVal })
      sessionStorage.removeItem('clientActiveTab')
      setSession({ token: data.token, email: emailVal, name: data.name, role: 'client', client_id: data.client_id })
      navigate('/client')
    } catch (err) {
  ...
  return (
    <AuthShell>
      ...
      <p style={{ color: 'var(--vfo-muted)', fontSize: '14px', marginBottom: '28px' }}>Welcome back — enter your client credentials.</p>
      ...
      <p style={{ color: 'var(--vfo-muted)', fontSize: '13px', marginTop: '20px', textAlign: 'center', cursor: 'pointer' }} onClick={()=>navigate('/forgot-password?type=client')}>Forgot passcode?</p>
