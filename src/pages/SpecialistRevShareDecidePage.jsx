...
export default function SpecialistRevShareDecidePage() {
  ...
  async function run() {
    ...
    try {
      const res = await fetch(API_URL, {
        ...
        body: JSON.stringify({ action: 'automation_SPECIALIST_revsharedecide', token, decision }),
      })
...
function getView(status, response) {
  ...
  if (response === 'Approved') return { icon: '✓', color: '#22c55e', title: 'Thank You!', message: 'Thank you for approving the revenue share proposal. We will be in touch with the next steps.' }
  return { icon: '✓', color: '#22c55e', title: 'Thank You', message: "Thank you — we've noted that you'd like to propose an edit, and we'll be in touch to discuss." }
}
