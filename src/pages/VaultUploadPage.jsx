...
// into the vault section the request was raised against. Mirrors /tax-upload.
async function uploadFile(token, file) {
  const tooBig = fileSizeError(file)
  if (tooBig) throw new Error(tooBig)
  const r1 = await fetch(API_URL, {
    method: 'POST', headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ action: 'vault_request_upload_url', token, filename: file.name }),
  })
  ...
  fetch(API_URL, {
    method: 'POST', headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ action: 'vault_request_upload_notify', token, file_name: file.name }),
  }).catch(() => {})
