  ...
  return (
    <div style={wrap}>
      <div style={{ marginBottom: '18px' }}>
        ...
        <h2 style={{ fontSize: '24px', fontWeight: 800, color: NAVY, margin: 0 }}>VFO Specialist Background Check Fees</h2>
      </div>
      ...
      {!loading && !error && (
        <div style={{ border: '1px solid #e9eef8', borderRadius: '14px', overflow: 'hidden', background: '#fff', boxShadow: '0 4px 16px rgba(20,45,95,0.06)' }}>
          ...
          {filtered.length === 0 && (
            <div style={{ textAlign: 'center', padding: '40px', color: '#9aa7be', fontSize: '14px' }}>No background check payments for this period.</div>
          )}
          {filtered.map(p => (
            <div key={p.id} style={{ display: 'grid', gridTemplateColumns: grid, gap: '8px', padding: '13px 18px', borderBottom: '1px solid #f0f3f9', alignItems: 'center', fontSize: '13px', color: '#16264a' }}>
              <span style={{ fontWeight: 600 }}>
                ...
                {p.bg_acct_last4 && <span style={{ color: '#9aa7be', fontWeight: 400 }}> · ••••{p.bg_acct_last4}</span>}
              </span>
              <span style={{ color: '#4e6087' }}>{p.background_check_type === 'Max' ? 'Max' : 'Core'}</span>
              ...
              <span style={{ textAlign: 'right', color: '#4e6087' }}>{fmtDate(p.bg_payment_completed_at)}</span>
            </div>
          ))}
        </div>
      )}
