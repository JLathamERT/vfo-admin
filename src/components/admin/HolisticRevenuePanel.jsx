...
//
// A split slice is money owed, not money moved: the member and strategic legs each carry
// a tiny note saying whether they have actually paid out, and an unpaid one renders
// dimmed. Elite VFO Income has no payout leg, but it is marked the same way while the
// payment itself is still clearing. Amounts and totals are the full split either way —
...
export default function HolisticRevenuePanel({ embedded = false }) {
  ...
  const grid = '110px 1.2fr 1.1fr 76px 104px 104px 104px 110px 100px'
  ...
  return (
    <div style={wrap}>
      ...
      {loading && <AccountingTableSkeleton cols={9} />}
      ...
      {!loading && !error && (
        <div style={{ border: '1px solid var(--vfo-border-soft)', borderRadius: '14px', overflow: 'hidden', background: 'var(--vfo-card)', boxShadow: 'var(--vfo-shadow-card)' }}>
          <div style={{ display: 'grid', gridTemplateColumns: grid, gap: '8px', padding: '12px 18px', background: 'var(--vfo-input)', borderBottom: '1px solid var(--vfo-border-soft)', fontSize: '10px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.6px', color: 'var(--vfo-muted)' }}>
            <span>Cleared</span><span>Client</span><span>Connected Member</span><span>Tier</span><span style={{ textAlign: 'right', borderRight: '1px solid var(--vfo-border-strong)', paddingRight: '12px' }}>Received</span><span style={{ textAlign: 'right' }}>Member Revenue Share</span><span style={{ textAlign: 'right' }}>Member Money Mapping</span><span style={{ textAlign: 'right' }}>Elite VFO Income</span><span style={{ textAlign: 'right' }}>Strategic Partner Revenue Share</span>
          </div>
