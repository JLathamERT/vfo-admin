      ...
      {activeTab === 'programs' && (
        <div>
          {PROGRAMS.map(p => {
            ...
            if (!dbProgram) {
              return (
                <div key={p.key} style={{ textAlign: 'center', padding: '40px', color: '#4e6087' }}>
            ...
            if (!isEnabled) {
              return (
                <div key={p.key} style={{ textAlign: 'center', padding: '60px 20px', color: '#4e6087' }}>
                  ...
                  <div style={{ fontSize: '14px', color: '#4e6087' }}>This program is not enabled for this member. Enable it from MSM Home.</div>
                </div>
              )
            }
      ...
      {activeTab === 'home' && (
        <div>
          ...
          <div style={sectionStyle}>
            ...
            <div style={{ display: 'flex', gap: '32px', flexWrap: 'wrap', marginBottom: '20px' }}>
              {[
                ...
                <div key={label} style={{ textAlign: 'center' }}>
                  <div style={{ fontSize: '28px', fontWeight: '700', color: '#16264a' }}>{count}</div>
                  <div style={{ fontSize: '11px', color: '#4e6087', marginTop: '2px', textTransform: 'uppercase' }}>{label}</div>
                </div>
              ))}
            </div>
            ...
            {showLogMeeting && (
              <div style={{ marginTop: '16px', padding: '16px', background: '#eef2f9', borderRadius: '8px' }}>
                <div style={{ display: 'flex', gap: '12px', marginBottom: '12px', flexWrap: 'wrap' }}>
                  <div style={{ flex: 1, minWidth: '160px' }}>
                    <label style={{ fontSize: '12px', color: '#4e6087', display: 'block', marginBottom: '6px' }}>Date *</label>
                    <input type="date" value={meetingDate} onChange={e => setMeetingDate(e.target.value)} style={inputStyle} />
                  </div>
                  <div style={{ flex: 1, minWidth: '160px' }}>
                    <label style={{ fontSize: '12px', color: '#4e6087', display: 'block', marginBottom: '6px' }}>Conducted By</label>
                    <select value={meetingConductedBy} onChange={e => setMeetingConductedBy(e.target.value)} style={{ ...inputStyle, background: '#ffffff' }}>
                ...
                <div style={{ marginBottom: '12px' }}>
                  <label style={{ fontSize: '12px', color: '#4e6087', display: 'block', marginBottom: '6px' }}>Notes</label>
                ...
                <div style={{ display: 'flex', gap: '8px' }}>
                  ...
                  <button onClick={() => setShowLogMeeting(false)} style={{ padding: '8px 20px', borderRadius: '8px', border: '1px solid #c7d4e8', background: 'transparent', color: '#4e6087', fontSize: '13px', cursor: 'pointer' }}>Cancel</button>
                </div>
          ...
          <div style={sectionStyle}>
            ...
            {meetings.length === 0
              ...
              : meetings.map(m => (
                <div key={m.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', padding: '10px 0', borderBottom: '1px solid #eef2f9' }}>
...
function EnrollPanel({ member, program, onEnrolled }) {
  ...
  return (
    <div style={sectionStyle}>
      <p style={{ color: '#4e6087', fontSize: '14px', marginBottom: '20px' }}>
      ...
      <div style={{ marginBottom: '12px', maxWidth: '200px' }}>
        <label style={{ fontSize: '12px', color: '#4e6087', display: 'block', marginBottom: '6px' }}>Date Joined</label>
  ...
  return (
    <div style={sectionStyle}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
        <div style={{ fontSize: '13px', color: '#4e6087', textTransform: 'uppercase', letterSpacing: '1px' }}>Notes ({notes.length})</div>
...
function EnrolledPanel({ member, enrollment, program, onDataChange }) {
  ...
  const statusColors = { 'On Fast Track': '#1b9254', 'Paused Fast Track': '#e06717', 'Lost/Removed': '#e74c3c', 'Revert to Legacy': '#4e6087', 'Active': '#1b9254', 'Paused': '#e06717', 'Not Renewing': '#e74c3c' }

  return (
    <div>
      <div style={{ fontFamily: 'Inter, sans-serif', fontWeight: 700, letterSpacing: '-0.02em', fontSize: '22px', color: '#16264a', marginBottom: '20px' }}>{program.name}</div>
      <div style={sectionStyle}>
          ...
          <div style={{ padding: '16px', background: '#eef2f9', borderRadius: '8px' }}>
            <div style={{ display: 'flex', gap: '12px', marginBottom: '12px', flexWrap: 'wrap' }}>
              <div style={{ flex: 1, minWidth: '160px' }}>
                ...
                <select value={programStatus} onChange={e => setProgramStatus(e.target.value)} style={{ ...inputStyle, background: '#ffffff' }}>
...
function TrainingTrack({ enrollment, program }) {
  ...
  if (phases.length === 0) return <div style={{ textAlign: 'center', padding: '40px', color: '#4e6087' }}>No training track defined for this program yet.</div>
  ...
  return (
    <div>
      <div style={{ display: 'flex', gap: '24px', marginBottom: '20px', flexWrap: 'wrap', paddingLeft: '24px' }}>
        <div style={{ textAlign: 'center' }}><div style={{ fontSize: '28px', fontWeight: '700', color: '#16264a' }}>{completedTasks}</div><div style={{ fontSize: '11px', color: '#4e6087' }}>COMPLETED</div></div>
        <div style={{ textAlign: 'center' }}><div style={{ fontSize: '28px', fontWeight: '700', color: '#16264a' }}>{totalTasks}</div><div style={{ fontSize: '11px', color: '#4e6087' }}>TOTAL</div></div>
        <div style={{ textAlign: 'center' }}><div style={{ fontSize: '28px', fontWeight: '700', color: '#1b9254' }}>{totalTasks > 0 ? Math.round(completedTasks / totalTasks * 100) : 0}%</div><div style={{ fontSize: '11px', color: '#4e6087' }}>PROGRESS</div></div>
      </div>
          ...
          <div key={phase.id} style={{ background: isReview ? '#f7f9fc' : '#ffffff', border: `1px solid ${borderColor}`, borderRadius: '14px', boxShadow: '0 3px 12px rgba(20,45,95,0.05)', marginBottom: isReview ? '10px' : '10px', marginTop: isReview ? '-6px' : '0', marginLeft: isReview ? '20px' : '0', overflow: 'hidden', borderTopLeftRadius: isReview ? '0' : '12px', borderTopRightRadius: isReview ? '12px' : '12px' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 18px' }}>
              <div onClick={() => setExpanded(p => ({ ...p, [phase.id]: !p[phase.id] }))} style={{ display: 'flex', alignItems: 'center', gap: '10px', cursor: 'pointer', flex: 1 }}>
                ...
                <span style={{ fontSize: '13px', fontWeight: '600', color: titleColor, textTransform: 'uppercase', letterSpacing: '0.5px' }}>{phase.name}{isReview && <span style={{ fontSize: '10px', color: '#4e6087', marginLeft: '8px', textTransform: 'none', fontWeight: '400', letterSpacing: '0' }}>checkpoint</span>}</span>
              </div>
            ...
            {isExpanded && (
              <div style={{ borderTop: `1px solid ${borderColor}`, padding: '12px 18px' }}>
                {tasks.map(task => {
                  ...
                  return (
                    <div key={task.id} style={{ padding: '8px 0', borderBottom: '1px solid #eef2f9', display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'wrap' }}>
                      ...
                      <div style={{ flex: 1, minWidth: '150px' }}>
                        <span style={{ fontSize: '14px', color: isDone ? '#4e6087' : '#16264a' }}>{task.name}</span>
                      </div>
                      <select value={p.status || ''} onChange={e => saveTask(task.id, e.target.value, p.completed_date, phase.id)} disabled={saving[task.id]} style={{ ...inputStyle, background: '#ffffff', minWidth: '130px', borderColor: isDone ? `${statusColor}66` : '#d6e0ee', color: isDone ? statusColor : '#16264a' }}>
    ...
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '20px', width: '100%' }}>
        <div style={{ display: 'flex', gap: '24px', alignItems: 'flex-end', paddingLeft: '24px' }}>
          <div><div style={{ fontSize: '28px', fontWeight: '700', color: '#16264a', lineHeight: 1 }}>{clients.length}</div><div style={{ fontSize: '11px', color: '#4e6087', marginTop: '4px' }}>TOTAL</div></div>
          <div><div style={{ fontSize: '28px', fontWeight: '700', color: '#16264a', lineHeight: 1 }}>{clients.filter(c => c.status === 'active').length}</div><div style={{ fontSize: '11px', color: '#4e6087', marginTop: '4px' }}>ACTIVE</div></div>
        </div>
      ...
      {showAdd && !isPFT && !addMode && (
        <div style={{ ...sectionStyle, marginBottom: '20px' }}>
          <div style={{ fontSize: '13px', color: '#4e6087', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '16px' }}>Add Client</div>
      ...
      {showAdd && addMode === 'existing' && (
        <div style={{ ...sectionStyle, marginBottom: '20px' }}>
          <div style={{ fontSize: '13px', color: '#4e6087', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '16px' }}>Add Existing Client</div>
          {loadingExisting
            ? <div style={{ padding: '20px', color: '#4e6087', textAlign: 'center' }}>Loading...</div>
            : allMemberClients.length === 0
              ? <div style={{ padding: '20px', color: '#4e6087', textAlign: 'center' }}>No other clients found for this member.</div>
              : <>
      ...
      {showAdd && addMode === 'new' && (
        <div style={{ ...sectionStyle, marginBottom: '20px' }}>
          ...
          <div style={{ display: 'flex', gap: '8px' }}>
            ...
            <button onClick={() => { isPFT ? setShowAdd(false) : setAddMode(null) }} style={{ padding: '8px 20px', borderRadius: '8px', border: '1px solid #c7d4e8', background: 'transparent', color: '#4e6087', fontSize: '13px', cursor: 'pointer' }}>{isPFT ? 'Cancel' : 'Back'}</button>
          </div>
          {addStatus && <p style={{ color: '#d93025', fontWeight: 500, fontSize: '13px', marginTop: '8px' }}>{addStatus}</p>}
        </div>
      )}
...
function ClientTrack({ client, program }) {
  ...
  if (phases.length === 0) return <div style={{ color: '#4e6087', fontSize: '13px', padding: '16px' }}>No client track defined for this program yet.</div>

  return (
    <div>
      {phases.map(phase => (
        <div key={phase.id} style={{ marginBottom: '20px' }}>
          <div style={{ fontSize: '13px', fontWeight: '600', color: '#0095ff', marginBottom: '10px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>{phase.name}</div>
            ...
            return (
              <div key={task.id} style={{ padding: '8px 0', borderBottom: '1px solid #f2f5fa', display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap' }}>
                ...
                <div style={{ flex: 1, minWidth: '140px' }}>
                  ...
                  <span style={{ fontSize: '13px', color: '#16264a' }}>{task.name}</span>
                </div>
                <select value={p.status || ''} onChange={e => saveTask(task.id, e.target.value, p.completed_date, p.completed_by, p.notes)} disabled={saving[task.id]} style={{ ...inputStyle, background: '#ffffff', minWidth: '130px' }}>
  ...
  return (
    <div>
      <div style={{ display: 'flex', gap: '32px', marginBottom: '20px', flexWrap: 'wrap', alignItems: 'flex-end', paddingLeft: '24px' }}>
        <div style={{ textAlign: 'center' }}><div style={{ fontSize: '28px', fontWeight: '700', color: '#16264a' }}>{completedCount}</div><div style={{ fontSize: '11px', color: '#4e6087' }}>COMPLETED</div></div>
        <div style={{ textAlign: 'center' }}><div style={{ fontSize: '28px', fontWeight: '700', color: '#0095ff' }}>{scheduledCount}</div><div style={{ fontSize: '11px', color: '#4e6087' }}>SCHEDULED</div></div>
        {nextScheduled && (
          <div style={{ textAlign: 'center' }}><div style={{ fontSize: '16px', fontWeight: '600', color: '#0095ff' }}>{nextScheduled.meeting_date.split('T')[0]}</div><div style={{ fontSize: '11px', color: '#4e6087' }}>NEXT MEETING</div></div>
        )}
      </div>

      <div style={{ marginBottom: '20px' }}>
        <button onClick={() => setShowLog(!showLog)} style={{ padding: '8px 20px', borderRadius: '8px', background: 'linear-gradient(135deg, #125ecc 0%, #0a85e8 100%)', border: 'none', boxShadow: '0 2px 8px rgba(18,94,204,0.28)', color: '#fff', fontSize: '13px', cursor: 'pointer' }}>+ Log Meeting</button>
      </div>
      ...
      {showLog && (
        <div style={{ ...sectionStyle, marginBottom: '20px' }}>
          <div style={{ display: 'flex', gap: '12px', marginBottom: '12px', flexWrap: 'wrap' }}>
            ...
            <div style={{ flex: 1, minWidth: '160px' }}>
              <label style={{ fontSize: '12px', color: '#4e6087', display: 'block', marginBottom: '6px' }}>Status</label>
              <select value={meetingStatus} onChange={e => setMeetingStatus(e.target.value)} style={{ ...inputStyle, background: '#ffffff' }}>
          ...
          <div style={{ display: 'flex', gap: '8px' }}>
            <button onClick={logMeeting} style={{ padding: '8px 20px', borderRadius: '8px', background: 'linear-gradient(135deg, #125ecc 0%, #0a85e8 100%)', border: 'none', boxShadow: '0 2px 8px rgba(18,94,204,0.28)', color: '#fff', fontSize: '13px', cursor: 'pointer' }}>Save</button>
            <button onClick={() => setShowLog(false)} style={{ padding: '8px 20px', borderRadius: '8px', border: '1px solid #c7d4e8', background: 'transparent', color: '#4e6087', fontSize: '13px', cursor: 'pointer' }}>Cancel</button>
          </div>
            ...
            <div key={m.id} style={{ borderBottom: '1px solid #eef2f9' }}>
              <div onClick={() => setExpandedMeeting(expandedMeeting === m.id ? null : m.id)}
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                  ...
                  <span style={{ fontSize: '11px', padding: '2px 8px', borderRadius: '4px', background: 'rgba(0,149,255,0.15)', color: '#0095ff', fontWeight: 600, border: '1px solid rgba(0,149,255,0.3)' }}>Scheduled</span>
                </div>
            ...
            <div key={m.id} style={{ borderBottom: '1px solid #eef2f9' }}>
              <div onClick={() => setExpandedMeeting(expandedMeeting === m.id ? null : m.id)}
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  {m.notes && <span style={{ fontSize: '11px', color: '#697a9c' }}>has notes</span>}
                  <span style={{ color: '#4e6087', fontSize: '10px', transform: expandedMeeting === m.id ? 'rotate(180deg)' : 'none', display: 'inline-block', transition: 'transform 0.2s' }}>▼</span>
                </div>
              </div>
              {expandedMeeting === m.id && (
...
function CoachingRenewal({ enrollment, member }) {
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: '20px' }}>
        <button onClick={() => setShowLog(!showLog)} style={{ padding: '8px 20px', borderRadius: '8px', background: 'linear-gradient(135deg, #125ecc 0%, #0a85e8 100%)', border: 'none', boxShadow: '0 2px 8px rgba(18,94,204,0.28)', color: '#fff', fontSize: '13px', cursor: 'pointer' }}>Log Renewal</button>
      </div>
      ...
      {showLog && (
        <div style={{ ...sectionStyle, marginBottom: '20px' }}>
          <div style={{ display: 'flex', gap: '12px', marginBottom: '12px', flexWrap: 'wrap' }}>
            <div style={{ flex: 1, minWidth: '160px' }}>
              <label style={{ fontSize: '12px', color: '#4e6087', display: 'block', marginBottom: '6px' }}>Action *</label>
              <select value={actionType} onChange={e => handleActionTypeChange(e.target.value)} style={{ ...inputStyle, background: '#ffffff' }}>
            ...
            {actionType === 'renewed' && (
              <div style={{ flex: 1, minWidth: '160px' }}>
                <label style={{ fontSize: '12px', color: '#4e6087', display: 'block', marginBottom: '6px' }}>Next Renewal Date</label>
      ...
      <div style={sectionStyle}>
        ...
        {renewals.length === 0
          ...
          : renewals.map(r => (
            <div key={r.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', padding: '12px 0', borderBottom: '1px solid #eef2f9' }}>
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                  <span style={{ fontSize: '11px', padding: '2px 8px', borderRadius: '4px', background: `${actionColors[r.action] || '#4e6087'}22`, color: actionColors[r.action] || '#4e6087', border: `1px solid ${actionColors[r.action] || '#4e6087'}44`, textTransform: 'capitalize' }}>{r.action}</span>
                ...
                {r.notes && <div style={{ fontSize: '12px', color: '#697a9c', marginTop: '2px' }}>{r.notes}</div>}
              </div>
              {r.created_by && <span style={{ fontSize: '11px', color: '#697a9c' }}>{r.created_by}</span>}
            </div>
          ))
        }
      </div>
