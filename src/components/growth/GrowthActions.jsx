...
const G2_OPTIONS = [
  { value: 'drop', label: 'Drop this Issue', color: '#697a9c' },
        ...
        return (
          <div key={cat} style={cardStyle}>
            ...
            <div style={{ padding: '16px 20px' }}>
              <div style={{ fontSize: '13px', fontWeight: 800, color: NAVY, letterSpacing: '-0.01em' }}>{CATEGORY_LABELS_LONG[cat]}</div>
...
function ActionRow({ r, update }) {
  return (
    <div style={{ padding: '14px 0', borderTop: '1px solid #eef2f9', display: 'flex', gap: '18px', alignItems: 'flex-start', flexWrap: 'wrap' }}>
      <div style={{ flex: '1 1 340px', minWidth: '240px', display: 'flex', gap: '10px' }}>
        <span style={{ width: '24px', height: '24px', borderRadius: '50%', background: '#eef2f9', border: '1px solid #dde5f2', color: MUTED, fontSize: '11px', fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, marginTop: '4px' }}>{r.action_number}</span>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '7px', flex: '0 0 auto' }}>
        {G2_OPTIONS.map(o => {
          ...
          return (
            <label key={o.value} style={{ display: 'flex', alignItems: 'center', gap: '7px', fontSize: '12.5px', cursor: 'pointer', fontWeight: on ? 700 : 500, color: on ? o.color : '#4e6087' }}>
