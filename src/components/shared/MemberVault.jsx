  ...
  return (
    <div>
      <div style={sectionStyle}>
        <div style={{ fontSize: '13px', color: '#4e6087', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '16px' }}>The Vault</div>
        ...
        {status && <p style={{ color: statusType === 'success' ? '#1b9254' : '#d93025', fontSize: '13px' }}>{status}</p>}
      </div>
      <div style={sectionStyle}>
        <div style={{ fontSize: '13px', color: '#4e6087', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '16px' }}>Your Files</div>
        {files.length === 0
          ? <p style={{ color: '#697a9c', fontSize: '14px' }}>No files uploaded yet.</p>
          : files.map(f => (
            <div key={f.name} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 0', borderBottom: '1px solid #eef2f9' }}>
              <span style={{ fontSize: '14px', color: '#16264a' }}>{f.name}</span>
              <div style={{ display: 'flex', gap: '8px' }}>
                <a href={f.url} target="_blank" rel="noreferrer" style={{ padding: '5px 12px', borderRadius: '6px', border: '1px solid #c7d4e8', color: '#4e6087', fontSize: '12px', textDecoration: 'none' }}>Download</a>
                <button onClick={() => deleteFile(f.name)} style={{ padding: '5px 12px', borderRadius: '6px', border: '1px solid rgba(231,76,60,0.3)', background: 'transparent', color: '#e74c3c', fontWeight: 600, fontSize: '12px', cursor: 'pointer' }}>Delete</button>
              </div>
            </div>
          ))
        }
