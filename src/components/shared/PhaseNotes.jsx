  ...
  return (
    <div onClick={e => e.stopPropagation()} style={{ borderTop: '1px solid var(--vfo-border-chip)', padding: '12px 18px', background: 'var(--vfo-tint)' }}>
      <div style={{ display: 'flex', gap: '8px', marginBottom: '10px' }}>
        <textarea value={newNote} onChange={e => setNewNote(e.target.value)} placeholder="Add a note..." rows={2} onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); addNote() } }} style={{ flex: 1, padding: '8px 12px', borderRadius: '8px', border: '1px solid var(--vfo-border-strong)', background: 'var(--vfo-input)', color: 'var(--vfo-ink)', fontSize: '13px', fontFamily: 'Inter, sans-serif', resize: 'vertical' }} />
      ...
      {phaseNotes.map(note => (
        <div key={note.id} style={{ padding: '8px 0', borderBottom: '1px solid var(--vfo-border-soft)' }}>
          {editingId === note.id ? (
